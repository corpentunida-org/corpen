...
Route::prefix('archivo')
    ...
    ->group(function () {
        ...
        Route::get('cargos/export-csv', [GdoCargoController::class, 'exportCsv'])->name('archivo.cargo.export.csv');

        // Ruta segura para ver manuales de funciones desde S3
        Route::get('cargos/{cargo}/ver-manual', [GdoCargoController::class, 'verManual'])->name('archivo.cargo.verManual');
        ...
        Route::get('empleados', [GdoEmpleadoController::class, 'index'])->name('archivo.empleado.index');

        Route::post('empleados', [GdoEmpleadoController::class, 'store'])->name('archivo.empleado.store');

        Route::put('empleados/{empleado}', [GdoEmpleadoController::class, 'update'])->name('archivo.empleado.update');

        Route::delete('empleados/{empleado}', [GdoEmpleadoController::class, 'destroy'])->name('archivo.empleado.destroy');

        // Visualización de Foto de Perfil (URL Firmada S3)
        Route::get('empleados/{id}/foto', [GdoEmpleadoController::class, 'verFoto'])->name('archivo.empleado.verFoto');

        // --- GESTIÓN DE DOCUMENTOS ADICIONALES (Implementación S3) ---
        // Subida de documentos adicionales
        Route::post('empleados/storeDocumento', [GdoEmpleadoController::class, 'storeDocumento'])->name('archivo.empleado.storeDocumento');

        // Rutas para ver y descargar documentos (ahora en GdoEmpleadoController)
        Route::get('empleados/verDocumento/{id}', [GdoEmpleadoController::class, 'verDocumento'])->name('archivo.empleado.verDocumento');

        Route::get('empleados/downloadDocumento/{id}', [GdoEmpleadoController::class, 'downloadDocumento'])->name('archivo.empleado.downloadDocumento');

        // Eliminación física y lógica de documentos adicionales
        Route::delete('empleados/documentos/{id}', [GdoEmpleadoController::class, 'destroyDocumento'])->name('archivo.empleado.destroyDocumento');

        // Ruta para el menú de navegación (apunta al mismo método index)
        Route::get('gdodocsempleados', [GdoEmpleadoController::class, 'index'])->name('archivo.gdodocsempleados.index');
        ...
        Route::post('funciones/asignar-cargo', [\App\Http\Controllers\Archivo\GdoFuncionController::class, 'asignarCargo'])->name('archivo.funcion.asignarCargo');

        // NUEVA RUTA: Cambiar estado Activo/Inactivo (Reemplaza a desvincular)
        Route::put('funciones/{cargo}/{funcion}/estado', [\App\Http\Controllers\Archivo\GdoFuncionController::class, 'cambiarEstadoVinculo'])->name('archivo.funcion.cambiarEstadoVinculo');
    });
...
Route::middleware('auth')
    ...
    ->group(function () {
        ...
        // 1. Generar PDF
        Route::get('workflows/{workflow}/pdf', [WorkflowController::class, 'generatePdf'])->name('workflows.pdf');

        // 2. ACTUALIZAR EQUIPO (AJAX) - ESTA ES LA RUTA NUEVA
        Route::put('workflows/{workflow}/update-team', [WorkflowController::class, 'updateTeam'])->name('workflows.updateTeam');
        ...
        // Auditoría
        Route::get('/auditoria/pdf', [AuditoriaProyectosController::class, 'exportPdf'])->name('auditoria.pdf');

        Route::get('/auditoria', [AuditoriaProyectosController::class, 'index'])->name('auditoria.index');
...
Route::middleware('auth')
    ...
    ->group(function () {
        ...
        Route::get('buscar-rapido', [ScpSoporteController::class, 'quickSearch'])->name('buscar.rapido');

        // 📂 Vistas filtradas predefinidas
        Route::get('pendientes', [ScpSoporteController::class, 'pendientes'])->name('pendientes');

        Route::get('mis-soportes', [ScpSoporteController::class, 'misSoportes']) // Ver tickets asignados a mí
            ->name('mis-soportes');

        Route::get('sin-asignar', [ScpSoporteController::class, 'sinAsignar'])->name('sinAsignar');

        // ⚙️ Acciones sobre el soporte
        Route::get('descargar/{id}', [ScpSoporteController::class, 'descargarSoporte'])->name('descargar');

        Route::get('ver/{id}', [ScpSoporteController::class, 'verSoporte'])->name('ver');

        // Asignar agente y Cambiar Estado (Acciones específicas)
        Route::post('{scpSoporte}/asignar', [ScpSoporteController::class, 'asignarAgente'])->name('asignar');

        Route::post('{scpSoporte}/cambiar-estado', [ScpSoporteController::class, 'cambiarEstado'])->name('cambiarEstado');

        // --- Resource Principal (CRUD Completo) ---
        Route::resource('soportes', ScpSoporteController::class)->parameters(['soportes' => 'scpSoporte']);

        // ---------------------------------------------------
        // 5. OBSERVACIONES Y ADJUNTOS
        // ---------------------------------------------------
        Route::post('soportes/{scpSoporte}/observaciones', [ScpSoporteController::class, 'storeObservacion'])->name('observaciones.store');

        Route::delete('soportes/{scpSoporte}/observaciones/{scpObservacion}', [ScpSoporteController::class, 'destroyObservacion'])->name('observaciones.destroy');

        // Descarga de adjuntos de observaciones
        Route::get('observaciones/adjunto/{id}', [ScpSoporteController::class, 'descargarAdjuntoObservacion'])->name('observaciones.adjunto');

        // ---------------------------------------------------
        // 6. FILTROS DINÁMICOS (AJAX SELECTS)
        // ---------------------------------------------------
        Route::get('tipos/filtro/{categoria}', [ScpSoporteController::class, 'getTiposByCategoria'])->name('tipos.byCategoria');
        ...
        Route::get('subtipos/filtro/{tipo}', [ScpSoporteController::class, 'getSubTipos'])->name('subtipos.byTipo');
...
Route::middleware(['auth'])
    ...
    ->group(function () {
        ...
        // Tablero Principal
        Route::get('tablero', [TableroInventarioController::class, 'index'])->name('tablero');

        // API para datos de gráficos (AJAX)
        Route::get('tablero/data', [TableroInventarioController::class, 'getChartData'])->name('tablero.data');
        ...
        Route::get('activos/{id}/hoja-vida', [ActivoController::class, 'hojaVida'])->name('activos.hoja_vida');
        ...
        // Catálogos Simples
        Route::resource('marcas', MarcaController::class)->parameters(['marcas' => 'invMarca']);

        Route::resource('bodegas', BodegaController::class)->parameters(['bodegas' => 'invBodega']);

        Route::resource('estados', EstadoController::class)->parameters(['estados' => 'invEstado']);

        Route::resource('metodos-pago', MetodoPagoController::class)->parameters(['metodos-pago' => 'invMetodo']);
        ...
        Route::resource('clasificacion', ClasificacionController::class)->parameters(['clasificacion' => 'invSubgrupo']);
        ...
        Route::get('clasificacion/lineas/{grupo_id}', [ClasificacionController::class, 'getLineasByGrupo'])->name('ajax.lineas');

        Route::get('clasificacion/subgrupos/{linea_id}', [ClasificacionController::class, 'getSubgruposByLinea'])->name('ajax.subgrupos');
    });
...
Route::middleware(['auth'])
    ...
    ->group(function () {
        ...
        Route::delete('procesos/{proceso}/remover-usuario/{user_id}', [ProcesoController::class, 'removerUsuario'])->name('procesos.removerUsuario');

        // --- NUEVAS RUTAS DE ESTADOS PARA PROCESOS ---
        Route::post('procesos/{proceso}/guardar-estado', [ProcesoController::class, 'guardarEstado'])->name('procesos.guardarEstado');

        Route::delete('procesos/estado/{id}', [ProcesoController::class, 'eliminarEstado'])->name('procesos.eliminarEstado');
        // ----------------------------------------------

        Route::get('ajax/usuarios-proceso/{proceso_id}', [ProcesoController::class, 'getUsuariosByProceso'])->name('ajax.usuarios.proceso');
        ...
        Route::post('notificaciones/{notificacion}/marcar-leida', [NotificacionController::class, 'marcarLeida'])->name('notificaciones.marcarLeida');

        // ---------------------------------------------------
        // 5. TRD (Tablas de Retención Documental)
        // ---------------------------------------------------
        Route::resource('trds', TrdController::class)->parameters(['trds' => 'trd']);

        // ---------------------------------------------------
        // 6. PLANTILLAS
        // ---------------------------------------------------
        Route::resource('plantillas', PlantillaController::class)->parameters(['plantillas' => 'plantilla']);

        // ---------------------------------------------------
        // 7. COMUNICACIONES DE SALIDA
        // ---------------------------------------------------
        Route::resource('comunicaciones-salida', ComunicacionSalidaController::class)->parameters(['comunicaciones-salida' => 'comunicacionSalida']);
...
Route::middleware('auth')
    ...
    ->group(function () {
        ...
        Route::get('/cliente/buscar', [VisitaCorpenController::class, 'buscarCliente'])->name('cliente.buscar');

        // Tablero general de visitas
        Route::get('tablero', [VisitaCorpenController::class, 'index'])->name('tablero');
