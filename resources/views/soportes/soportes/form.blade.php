...
<div class="card">
    <div class="card-header bg-gradient-primary text-white p-4">
        ...
        <p class="mb-0 opacity-75 small">
            {{ $modoEdicion ? 'Modifica la información del soporte existente' : 'Completa el formulario para crear un nuevo soporte' }}
        </p>
    </div>
    <div class="card-body p-4">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                ...
                <div class="form-floating">
                    <select class="form-select select2 @error('id_categoria') is-invalid @enderror" id="id_categoria"
                        name="id_categoria" {{ $modoEdicion ? 'disabled' : '' }} required>
            ...
            <div class="col-md-4">
                ...
                <div class="form-floating">
                    <select class="form-select select2 @error('id_scp_tipo') is-invalid @enderror" id="id_scp_tipo"
                        name="id_scp_tipo" {{ $modoEdicion ? 'disabled' : '' }} required>
                        @if ($modoEdicion)
            ...
            <div class="col-md-4">
                ...
                <div class="form-floating">
                    <select class="form-select select2 @error('id_scp_sub_tipo') is-invalid @enderror"
                        id="id_scp_sub_tipo" name="id_scp_sub_tipo" {{ $modoEdicion ? 'disabled' : '' }} required>
                        @if ($modoEdicion)
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-8">
                ...
                <textarea class="form-control @error('detalles_soporte') is-invalid @enderror" id="detalles_soporte"
                    name="detalles_soporte" rows="4" placeholder="Describe detalladamente tu solicitud..."
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-person-circle me-1 text-primary"></i>
                    Solicitante</label>
            ...
            <div class="col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-diagram-3 me-1 text-primary"></i> Cargo /
                    Área</label>
                <div class="input-group">
                    <span class="input-group-text"><i class="bi bi-briefcase"></i></span>
                    <input type="text" class="form-control bg-light"
                        value="{{ $usuario->cargo->nombre_cargo ?? 'Sin cargo asignado' }}" readonly>
            ...
            <div class="card border-0 bg-light">
                <div class="card-body p-3">
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <label for="id_cre_lineas_creditos" class="form-label fw-semibold mb-0">Línea de
                            Crédito</label>
                    </div>
                    <select class="form-select select2" id="id_cre_lineas_creditos" name="id_cre_lineas_creditos"
                    ...
                    @if ($modoEdicion)
                        <input type="hidden" name="id_cre_lineas_creditos"
                            value="{{ $soporte->id_cre_lineas_creditos ?? '' }}">
                    @endif
                </div>
            </div>
        </div>
                ...
                <div class="mt-3">
                    <div class="alert alert-success d-flex align-items-center" role="alert">
                        ...
                        <div>
                            ...
                            <div class="mt-2">
                                <a href="{{ route('soportes.ver', $soporte->id) }}" target="_blank"
                                    class="btn btn-sm btn-outline-primary me-2"><i class="bi bi-eye me-1"></i> Ver</a>
                                <a href="{{ route('soportes.descargar', $soporte->id) }}"
                                    class="btn btn-sm btn-outline-success"><i class="bi bi-download me-1"></i>
                                    Descargar</a>
                            </div>
                        </div>
                    </div>
        ...
        $(document).ready(function() {
            ...
            function cargarTipos(categoriaId) {
                ...
                if (categoriaId) {
                    ...
                    let url = "{{ route('soportes.tipos.byCategoria', ':categoria') }}".replace(':categoria',
                        categoriaId);
                    $.ajax({
                        url: url,
                        type: 'GET',
                        ...
                        success: function(data) {
                            tipoSelect.prop('disabled', false).empty().append(
                                '<option value="">Selecciona un tipo</option>');
            ...
            function cargarSubTipos(tipoId) {
                ...
                if (tipoId) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        success: function(data) {
                            subTipoSelect.prop('disabled', false).empty().append(
                                '<option value="">Selecciona un sub-tipo</option>');
                            $.each(data, function(key, value) {
                                subTipoSelect.append(
                                    `<option value="${value.id}">${value.nombre}</option>`);
                            });
