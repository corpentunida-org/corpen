                    ...
                    <form method="POST" action="{{ route('maestras.terceros.update', $tercero->cod_ter) }}">
                        ...
                        <div class="accordion" id="accordionTercero">
                            @php
                                ...
                                $ignore = ['id', 'cod_ter'];
                                $groups = [
                                    // -- INICIO DE LA MODIFICACIÓN EN EL ARRAY --
                                    'Información Personal' => ['nom_ter','estado','apl1','apl2','nom1','nom2','apell1','apell2','sexo','fec_nac','est_civil','tipo_ter','tip_pers','tdoc','dv','digito_v','razon_soc','raz','nom_conyug','id_conyuge','parentezco','mail_conyu','num_hijos','fec_falle','contacto','cont_tel','cargo', 'tip_prv'], // <-- AÑADIDO AQUÍ
                                    ...
                                    'Tributaria' => ['aut_ret','ret_iva','ret_ica','cod_respfiscal','cod_tributo','codimpuesto','codpostal','Cod_acteco','fec_minis','cod_dist','cod_est','inf_ter'], // <-- ELIMINADO DE AQUÍ ('ret_prv' se mantiene si es otro campo, 'tip_prv' se quita)
                                    ...
                                    // -- FIN DE LA MODIFICACIÓN EN EL ARRAY --
                                ];
                            @endphp
                                        ...
                                        @foreach($names as $field)
                                            @if(!in_array($field, $ignore))
                                                <div class="col-md-4 mb-3">
                                                    ...
                                                    {{-- Esta lógica ahora se ejecutará en la sección "Información Personal" --}}
                                                    @if ($field === 'tip_prv')
                                                        <select class="form-select" name="tip_prv">
                                                            <option value="">-- Seleccione un tipo --</option>
                                                            @foreach ($tipos as $tipo)
                                                                <option value="{{ $tipo->codigo }}" @selected(old('tip_prv', $tercero->tip_prv) == $tipo->codigo)>
                                                                    {{ $tipo->nombre }}
                                                                </option>
                                                            @endforeach
                                                        </select>
