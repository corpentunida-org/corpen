                                        ...
                                        @foreach ($columna as $plan)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="planes[]"
                                                    value="{{ $plan['valor'] }}" id="plan{{ $plan['valor'] }}">
                                                <label class="form-check-label" for="plan{{ $plan['valor'] }}">
        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @else
                            <form action="{{ route('seguros.beneficios.store') }}" method="post" id="formAddBeneficios" class="row"
                                novalidate>
                                @method('POST')
                                @csrf
                                <div class="row">
                                        ...
                                        <tbody>
                                            @foreach ($listadata as $i)
                                                <tr>
                                                    ...
                                                    <td><span class="badge bg-gray-200 text-dark">{{ $i->asegurado->parentesco }}</span>
                                                    </td>
                                                    <td><a>{{$i->tercero->edad}}</a></td>
                                                    <td>
                                                        <a><span class="fs-12 fw-normal text-muted"> {{$i->plan->name}} - </span>
                                                            ${{ number_format($i->plan->valor) }}</a>
                                                        <p class="fs-12 text-muted text-truncate-1-line tickets-sort-desc">Valor Prima:
