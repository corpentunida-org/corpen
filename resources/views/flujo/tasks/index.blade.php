<x-base-layout>
    <div class="task-index-wrapper">
        ...
        <div class="index-toolbar">
            <form action="{{ route('flujo.tasks.index') }}" method="GET" id="taskFilterForm"
                class="filters-container-corp">
                <div class="search-input-wrapper">
                    <i class="fas fa-search"></i>
                    <input type="text" name="search" id="liveSearchTasks"
                        placeholder="Buscar por título o descripción..." value="{{ request('search') }}"
                        autocomplete="off">
                </div>
                        ...
                        <select name="workflow_id" class="auto-filter-task filter-workflow-highlight">
                            ...
                            @foreach ($workflows as $workflow)
                                <option value="{{ $workflow->id }}"
                                    {{ request('workflow_id') == $workflow->id ? 'selected' : '' }}>
                                    {{ $workflow->nombre }}
                                </option>
                            @endforeach
                        </select>
            ...
            @fragment('tasks-list')
                            ...
                            <tbody>
                                @forelse($tasks as $task)
                                    <tr class="task-row">
                                        {{-- CLIC EN ID -> VER --}}
                                        <td>
                                            <a href="{{ route('flujo.tasks.show', $task->id) }}" class="id-link-badge"
                                        ...
                                        <td>
                                            <div class="task-info-cell">
                                                ...
                                                <a href="{{ route('flujo.tasks.edit', $task) }}"
                                                    class="task-title-main action-edit-link" title="Editar Unidad">
                                        ...
                                        <td>
                                            ...
                                            @if ($task->workflow)
                                                <a href="{{ route('flujo.workflows.show', $task->workflow->id) }}"
                                                    class="workflow-link-cell" title="Ir al Proyecto">
                                        ...
                                        <td>
                                            <span class="d-flex align-items-center gap-1">
                                                <li
                                                    class="text-{{ $task->prioridad == 'alta' ? 'danger' : ($task->prioridad == 'media' ? 'warning' : 'success') }} fs-5">
                                                </li>
                                        ...
                                        <td>                                           
                                            <div style="display: flex; align-items: center; gap: 6px;">
                                                <div
                                                    style="width: 20px; height: 20px; border-radius: 50%; background: #e8eaed; color: #3c4043; display: flex; align-items: center; justify-content: center; font-size: 0.6rem; font-weight: bold;">
                                        ...
                                        <td class="text-right">
                                            <div class="actions-group d-flex gap-2">
                                                <a href="{{ route('flujo.tasks.show', $task->id) }}" class="btn-icon-table"
                                                    title="Consultar">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                @candirect('flujot.tarea.editar')
                                                <a href="{{ route('flujo.tasks.edit', $task->id) }}" class="btn-icon-table"
                                                    title="Gestionar">
                    ...
                    <div class="mobile-tasks-grid d-only-mobile">
                        @forelse($tasks as $task)
                            <div class="mobile-task-card">
                                <div class="card-top">
                                    <a href="{{ route('flujo.tasks.show', $task->id) }}" class="m-id-tag">ID
                                        #{{ $task->id }}</a>
                                    <span
                                        class="badge-status-corp {{ $statusStyle ?? 'st-default' }}">{{ ucfirst($task->estado) }}</span>
                                </div>
                                <div class="card-mid">
                                    <a href="{{ route('flujo.tasks.edit', $task) }}"
                                        class="m-task-title action-edit-link">{{ $task->titulo }}</a>

                                    {{-- CONTEXTO DE WORKFLOW EN MÓVIL CON LINK --}}
                                    @if ($task->workflow)
                                        <a href="{{ route('flujo.workflows.show', $task->workflow->id) }}"
                                            class="m-workflow-tag">
                ...
                <footer class="index-footer">
                    <div class="pagination-info">
                        Mostrando <strong>{{ $tasks->firstItem() }}-{{ $tasks->lastItem() }}</strong> de
                        {{ $tasks->total() }}
                    </div>
    ...
    <style>
        ...
        .workflow-link-cell:hover .icon-mini {
            opacity: 1;
        }
        ...
        .skeleton-box {
            height: 15px;
            background: linear-gradient(90deg, #f0f3f5 25%, #e6e9ef 50%, #f0f3f5 75%);
            ...
            const performFilter = async () => {
                ...
                try {
                    ...
                    ajaxContainer.innerHTML =
                        '<div class="error-msg text-center p-4">Error de conexión. Reintente.</div>';
                }
            };
