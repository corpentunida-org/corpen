<x-base-layout>
    ...
    <div class="app-container">
        ...
        <div id="view-dashboard" class="tab-content {{ $activeTab === 'dashboard' ? 'active' : '' }}">
            ...
            <div class="kpi-strip">
                ...
                @foreach (['activo', 'pausado', 'completado', 'archivado'] as $est)
                    @php
                        $color = match ($est) {
                            'activo' => '#10b981', // Verde
                    ...
                    @endphp
                    <div class="kpi-box clickable" onclick="applyFilter('estado', '{{ $est }}')"
        ...
        <div id="view-gestion" class="tab-content {{ $activeTab === 'gestion' ? 'active' : '' }}">
            ...
            <div class="toolbar-panel">
                <form action="{{ route('flujo.workflows.index') }}" method="GET" id="main-filter-form"
                    class="toolbar-form">
                    ...
                    <div class="search-group">
                        <i class="fas fa-search" onclick="document.getElementById('main-filter-form').submit();"
                            style="cursor: pointer;" title="Buscar"></i>
                        ...
                        @if (request()->filled('search'))
                            <a href="{{ route('flujo.workflows.index') }}" class="clear-search"
                                title="Limpiar búsqueda"><i class="fas fa-times"></i></a>
                        @endif
                    </div>
                    ...
                    <div class="filters-group">
                        <div class="status-legend">
                            @if (request('estado'))
                                <span class="active-filter-badge">
                                    ...
                                    <a href="#" onclick="applyFilter('estado', ''); return false;"><i
                            ...
                            @if (request('condicion'))
                                <span class="active-filter-badge warning">
                                    ...
                                    <a href="#" onclick="applyFilter('condicion', ''); return false;"><i
                                            class="fas fa-times"></i></a>
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="leader-select-wrapper">
                            <select id="leader-select" onchange="applyFilter('user', this.value)"
                                placeholder="Filtrar por Líder...">
                                <option value="">Todos los líderes</option>
                                @foreach ($users as $u)
                                    <option value="{{ $u->id }}"
                                        {{ request('asignado_a') == $u->id ? 'selected' : '' }}>{{ $u->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="projects-list">
                ...
                @forelse($workflows as $wf)
                    ...
                    <div class="project-row {{ $isOverdue ? 'row-overdue' : '' }}">
                            ...
                            <div class="progress-container-neo">
                                <div class="progress-track">
                                    ...
                                    <div class="progress-fill"
                                        style="width: {{ $calculatedProgress }}%; background: {{ $progColor }};">
                                    </div>
                                </div>
                        ...
                        <div class="col-meta">
                            <div class="meta-item">
                                <div class="mini-avatar" title="{{ $wf->asignado?->name ?? 'Sin asignar' }}">
                                    {{ substr($wf->asignado?->name ?? '?', 0, 1) }}</div>
                    ...
                    <table style="width: 100%; border-collapse: collapse; min-width: 800px;">
                        <thead>
                            <tr style="background: #f8fafc; border-bottom: 2px solid var(--border); text-align: left;">
                                <th
                                    style="padding: 15px 20px; font-size: 12px; font-weight: 700; color: var(--text-gray);">
                                    PROYECTO</th>
                                <th
                                    style="padding: 15px 20px; font-size: 12px; font-weight: 700; color: var(--text-gray);">
                                    FECHA INICIO</th>
                        ...
                        <tbody>
                            @forelse($workflows as $wf)
                                ...
                                <tr style="border-bottom: 1px solid var(--border);">
                                    <td style="padding: 15px 20px;">
                                        <div style="font-weight: 600; color: var(--text-dark);">{{ $wf->nombre }}
                                        </div>
                                        <div style="font-size: 11px; color: var(--text-gray);">{{ $wf->estado }}
                                        </div>
                                    </td>
                                    ...
                                    <td style="padding: 15px 20px;">
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                            <div
                                                style="flex: 1; height: 6px; background: #f1f5f9; border-radius: 4px; overflow: hidden; width: 80px;">
                                                <div
                                                    style="height: 100%; width: {{ $progWidth }}%; background: {{ $progColor }};">
                                                </div>
                                            </div>
                                            <span
                                                style="font-size: 12px; font-weight: 700; color: {{ $progColor }};">{{ $progWidth }}%</span>
                                        </div>
                                    </td>
                                    <td style="padding: 15px 20px;">
                                        <span
                                            style="display: inline-flex; align-items: center; gap: 6px; padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; background-color: {{ $timeColor }}15; color: {{ $timeColor }}; border: 1px solid {{ $timeColor }}30;">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5"
                                        style="padding: 40px; text-align: center; color: var(--text-gray);">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $data = [
                $rawCounts['activo'] ?? 0,
                $rawCounts['archivado'] ?? 0,
                $rawCounts['pausado'] ?? 0,
                $rawCounts['completado'] ?? 0,
            ];
            @endphp
            ...
            if (ctxDist) {
                new Chart(ctxDist, {
                    type: 'doughnut',
                    data: {
                        labels: {!! json_encode(array_keys($estadoLabels)) !!},
                        keys: {!! json_encode(array_keys($estadoLabels)) !!},
                        datasets: [{
                            data: {!! json_encode($data) !!},
                            backgroundColor: [ '#55D58F','#E9ECEF','#FFA21D','#3454D1',],
                            borderWidth: 0
                        }]
                    },
                    options: {
                    ...
                    options: {
                        ...
                        onClick: (evt, elements) => {
                            if (elements.length > 0) {
                                ...
                                const key = evt.chart.data.keys[index];
                                if (key === 'completado') {
                                    applyFilter('estado', 'completado');
                                } else {
                                    applyFilter('condicion', key);
                                }
                            }
                        }
                    }
