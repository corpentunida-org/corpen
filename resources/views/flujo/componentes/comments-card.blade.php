...
<div class="corporate-comments-card">
    <style>
        ...
        .comment-table-minimal td { padding: 18px 15px; font-size: 0.85rem; border-bottom: 1px solid #f1f5f9; vertical-align: top; }
        .comment-table-minimal tbody tr:hover { background: #fcfdfe; }

        /* Contenido del Mensaje */
        .comment-text-wrapper { max-width: 500px; }
        .comment-main-text { 
            display: block; 
            color: #0f172a; 
            line-height: 1.6; 
        ...
        .trace-container { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
        
        .project-tag-mini {
            font-size: 0.65rem; font-weight: 700; color: #475569; background: #f1f5f9;
            padding: 3px 10px; border-radius: 6px; display: inline-flex; align-items: center; gap: 6px;
            border: 1px solid #e2e8f0;
        }

        .comment-task-ref { 
            display: inline-flex; align-items: center; gap: 5px; 
            font-size: 0.75rem; color: #4f46e5; text-decoration: none; font-weight: 700;
        }
        ...
        .user-avatar-init { 
            ...
            font-size: 0.75rem; font-weight: 800; border: 1px solid #e0e7ff;
        }
        
        .timestamp-corp { font-size: 0.75rem; color: #94a3b8; white-space: nowrap; text-align: right; }
        .ref-tag { font-family: 'Outfit'; font-size: 0.7rem; color: #64748b; background: #f8fafc; padding: 3px 8px; border-radius: 6px; font-weight: 700; border: 1px solid #f1f5f9; }

        /* --- MÓVIL (Adaptive Timeline) --- */
        .mobile-comments-view { display: none; flex-direction: column; gap: 16px; }

        @media (max-width: 900px) {
            .comment-table-minimal { display: none; }
            .mobile-comments-view { display: flex; }
            .corporate-comments-card { padding: 15px 10px; }
            ...
            .m-user-block { display: flex; align-items: center; gap: 10px; }
            .m-user-name { font-size: 0.85rem; font-weight: 700; color: #0f172a; }
            .m-time { font-size: 0.7rem; color: #94a3b8; }
                ...
                <td>
                    <div class="user-info-group">
                        ...
                        <div style="display: flex; flex-direction: column;">
                            <span style="font-weight: 700; color: #0f172a; font-size: 0.85rem;">{{ $comment->user->name ?? 'Sistema' }}</span>
                            <span style="font-size: 0.65rem; color: #64748b; text-transform: lowercase;">{{ $comment->user->email ?? '' }}</span>
                        </div>
                    </div>
                </td>
                <td class="timestamp-corp">
                    <div style="color: #0f172a; font-weight: 700;"><i class="far fa-clock"></i> {{ $comment->created_at->diffForHumans() }}</div>
                    <div style="font-size: 0.65rem; margin-top: 4px; font-weight: 500;">{{ $comment->created_at->format('d/m/Y H:i') }}</div>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="4" style="text-align:center; padding: 80px; color: #94a3b8;">
                    <i class="far fa-comment-dots" style="display: block; font-size: 3rem; margin-bottom: 20px; opacity: 0.1;"></i>
        ...
        <div class="comment-mobile-card">
            <div class="m-comment-header">
                <div class="m-user-block">
                    <div class="user-avatar-init" style="width: 28px; height: 28px; font-size: 0.65rem;">{{ substr($comment->user->name ?? 'S', 0, 1) }}</div>
                    <div>
            ...
            <div class="m-trace-footer">
                ...
                <div class="m-ref-row">
                    ...
                    <span class="m-time" style="font-size: 0.6rem;">{{ $comment->created_at->format('d/m/y H:i') }}</span>
                </div>
            </div>
        </div>
        @empty
