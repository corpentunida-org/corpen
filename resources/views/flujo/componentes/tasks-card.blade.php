...
<div class="corporate-task-card">
    <style>
        ...
        .task-table-minimal tbody tr:hover {
            background: #fcfdfe;
        }
    ...
    <div class="task-toolbar">
        <div class="compact-search">
            ...
            <input type="text" onkeyup="filtrarContenido(this, 'moduloTareas')" placeholder="Filtrar tareas..."
                autocomplete="off">
        </div>
    </div>
        ...
        <tbody>
            @forelse($tasks as $task)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('flujo.tasks.show', $task->id) }}" class="btn-act-icon" title="Consultar"><i
                                class="fas fa-eye"></i></a>
                        @candirect('flujot.tarea.editar')
                        <a href="{{ route('flujo.tasks.edit', $task->id) }}" class="btn-act-icon" title="Editar"><i
                                class="fas fa-edit"></i></a>@endcandirect
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="6" style="text-align:center; padding: 60px; color: #94a3b8;">
                        <i class="fas fa-tasks"
                            style="font-size: 2rem; display: block; margin-bottom: 10px; opacity: 0.2;"></i>
            ...
            <div class="task-mobile-card">
                ...
                <div class="m-task-details">
                    ...
                    <div class="m-detail-item">
                        <label>Plazo</label>
                        <span
                            style="font-size: 0.8rem; font-weight: 600;">{{ $task->fecha_limite ? $task->fecha_limite->format('d/m/y') : 'S/F' }}</span>
                    </div>
                    <div class="m-detail-item">
                        <label>Workflow</label>
                        <span
                            style="font-size: 0.75rem; color: #4f46e5; font-weight: 600;">{{ Str::limit($task->workflow->nombre ?? 'N/A', 15) }}</span>
                    </div>
                    <div class="m-detail-item">
                        <label>Responsable</label>
                        <div class="resp-cell">
                            <div class="resp-avatar" style="width: 20px; height: 20px; font-size: 0.5rem;">
                                {{ substr($task->user->name ?? '?', 0, 1) }}</div>
                            <span
                                style="font-size: 0.8rem; font-weight: 600;">{{ explode(' ', $task->user->name ?? 'N/A')[0] }}</span>
                        </div>
                    </div>
                </div>
