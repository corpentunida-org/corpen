                        ...
                        <div class="accordion-item">
                            ...
                            <div class="accordion-content" id="nombre-filter">
                                <div class="filter-group">
                                    <input type="text" name="nombre" id="filter-nombre"
                                        placeholder="Nombre específico..." value="{{ request('nombre') }}">
                                </div>
                            </div>
                        </div>
                        {{-- Acordeón 2: Estado --}}
                                    ...
                                    <div class="filter-group">
                                        <select name="estado" id="filter-estado">
                                            ...
                                            @foreach ($estados as $key => $value)
                                                <option value="{{ $key }}"
                                                    {{ request('estado') == $key ? 'selected' : '' }}>{{ $value }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        @if (isset($users) && count($users) > 0)
                                            @foreach ($users as $user)
                                                <option value="{{ $user->id }}"
                                                    {{ request('asignado_a') == $user->id ? 'selected' : '' }}>
                                                    {{ $user->name }}</option>
                                            @endforeach
                                        @else
        ...
        <div class="projects-grid-soft" id="workflows-grid">
            @forelse ($workflows as $workflow)
                ...
                <div class="project-item-card {{ $workflow->estado === 'archivado' ? 'is-archived' : '' }}"
                    data-name="{{ strtolower($workflow->nombre) }}">
                    <div class="p-card-body">
                        <div class="p-card-top">
                            <span class="p-priority-dot {{ $workflow->prioridad }}"
                                title="Prioridad: {{ ucfirst($workflow->prioridad) }}"></span>
                            <h3 class="p-name">
                                <a
                        ...
                        <p class="p-description">
                            {{ Str::limit($workflow->descripcion, 70) ?? 'Sin descripción disponible.' }}</p>
                    ...
                    <div class="p-card-footer">
                        <div class="p-footer-info">
                            ...
                            <div class="p-dates">
                                ...
                                {{ $workflow->fecha_fin?->format('d M') ?? 'S/F' }}
                            </div>
                        </div>
                        <div class="p-user">
                            <div class="user-avatar-mini"
                                title="Asignado a: {{ $workflow->asignado->name ?? 'Sin asignar' }}"
                                style="background: {{ $workflow->asignado_a ? '#10b981' : '#64748b' }}">
...
<style>
    ...
    .project-item-card:hover .p-actions-hidden {
        opacity: 1;
    }
