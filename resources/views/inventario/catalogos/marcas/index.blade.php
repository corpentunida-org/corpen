    ...
    <div class="cat-wrapper">
        ...
        <div class="cat-card">
            ...
            <form action="{{ route('inventario.marcas.store') }}" method="POST" class="cat-form">
                ...
                <textarea name="descripcion" class="cat-textarea" placeholder="Descripción de la marca (opcional)"></textarea>
            ...
            <div>
                ...
                <div class="cat-item">
                    <div>
                        <div class="cat-name">{{ $marca->nombre }}</div>
                        @if($marca->descripcion)
                        <div class="cat-desc">{{ $marca->descripcion }}</div>
                        @endif
                    </div>
                    <div class="cat-actions">
                        <span style="color: #cbd5e1; margin-right: 8px;">ID: {{ $marca->id }}</span>
                        <button class="cat-btn-edit" onclick="openEditModal({{ $marca->id }}, '{{ $marca->nombre }}', '{{ $marca->descripcion ?? '' }}')">Editar</button>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="modal-content">
            ...
            <form id="editForm" action="#" method="POST">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <textarea id="editDescripcion" name="descripcion" class="cat-textarea" style="width: 100%; margin-top: 5px;"></textarea>
                </div>
