<x-base-layout>
    <style>
        :root { --exec-text: #0f172a; --exec-muted: #64748b; --exec-border: #e2e8f0; --exec-bg: #f8fafc; --exec-primary: #4f46e5; }
        .dashboard-wrapper { font-family: 'Inter', sans-serif; color: var(--exec-text); max-width: 1200px; margin: 0 auto; padding: 30px; }
        
        .welcome-bar { display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 40px; }
        .system-tag { font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: var(--exec-primary); letter-spacing: 0.1em; display: block; margin-bottom: 5px; }
        .main-title { font-size: 2rem; font-weight: 800; margin: 0; line-height: 1; letter-spacing: -0.03em; }
        .main-subtitle { font-size: 0.95rem; color: var(--exec-muted); margin-top: 5px; }
        
        /* --- BOTONES MEJORADOS --- */
        .header-actions { display: flex; gap: 12px; }
        .btn-alert { background: #dc2626; color: #fff; padding: 10px 20px; border-radius: 8px; font-weight: 600; font-size: 0.85rem; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; transition: 0.2s; }
        .btn-alert:hover { background: #b91c1c; }
        .btn-primary { background: var(--exec-primary); color: #fff; padding: 10px 20px; border-radius: 8px; font-weight: 600; font-size: 0.85rem; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; transition: 0.2s; box-shadow: 0 4px 6px -1px rgba(79, 70, 229, 0.2); }
        .btn-primary:hover { background: #4338ca; transform: translateY(-1px); }
        
        /* --- KPIs ESTILO TARJETA (NUEVO) --- */
        .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 24px; margin-bottom: 40px; }
        .stat-card { background: #fff; border: 1px solid var(--exec-border); border-radius: 16px; padding: 24px; display: flex; flex-direction: column; position: relative; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.02); transition: transform 0.2s, box-shadow 0.2s; }
        .stat-card:hover { transform: translateY(-3px); box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.05); }
        .stat-label { font-size: 0.75rem; font-weight: 700; color: var(--exec-muted); text-transform: uppercase; letter-spacing: 0.05em; z-index: 1; }
        .stat-num { font-size: 2.2rem; font-weight: 800; line-height: 1.1; margin-top: 8px; z-index: 1; }
        .stat-icon { position: absolute; right: -10px; bottom: -15px; font-size: 5rem; color: #f1f5f9; z-index: 0; transform: rotate(-15deg); transition: 0.3s; }
        .stat-card:hover .stat-icon { transform: rotate(0deg) scale(1.1); color: #e2e8f0; }

        .dashboard-columns { display: grid; grid-template-columns: 1fr 2fr; gap: 24px; align-items: start; }
        .chart-container { position: relative; height: 280px; width: 100%; padding: 20px; }

        .card-stack { background: #fff; border: 1px solid var(--exec-border); border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.02); }
        .card-header { padding: 20px 30px; border-bottom: 1px solid var(--exec-border); display: flex; justify-content: space-between; align-items: center; }
        .card-title { font-weight: 700; display: flex; align-items: center; gap: 12px; }
        .icon-sq { width: 36px; height: 36px; background: #eef2ff; color: var(--exec-primary); border-radius: 8px; display: flex; align-items: center; justify-content: center; }
        
        .table-min { width: 100%; border-collapse: collapse; font-size: 0.9rem; }
        .table-min th { text-align: left; padding: 15px 30px; color: var(--exec-muted); font-size: 0.75rem; text-transform: uppercase; border-bottom: 1px solid var(--exec-border); font-weight: 600; }
        .table-min td { padding: 15px 30px; border-bottom: 1px solid var(--exec-border); vertical-align: middle; transition: background 0.1s; }
        
        /* --- HOVER EN LA TABLA (NUEVO) --- */
        .table-min tbody tr:hover td { background-color: #f8fafc; cursor: pointer; }
        
        .badge { padding: 4px 10px; border-radius: 20px; font-size: 0.7rem; font-weight: 700; display: inline-block; }
        .bg-green { background: #dcfce7; color: #166534; }
        .bg-gray { background: #f1f5f9; color: #475569; }

        @media (max-width: 992px) {
            .dashboard-columns { grid-template-columns: 1fr; }
            .stats-grid { grid-template-columns: repeat(2, 1fr); }
            .welcome-bar { flex-direction: column; align-items: flex-start; gap: 20px; }
        }
        @media (max-width: 576px) {
            .stats-grid { grid-template-columns: 1fr; }
            .header-actions { flex-direction: column; width: 100%; }
            .header-actions a { justify-content: center; }
        }
    </style>

    <div class="dashboard-wrapper">
        <div class="welcome-bar">
            <div>
                <span class="system-tag">Inteligencia de Negocios</span>
                <h1 class="main-title">Tablero de Activos</h1>
                <p class="main-subtitle">Resumen ejecutivo del estado del inventario.</p>
            </div>
            
            <div class="header-actions">
                <a href="{{ route('inventario.movimientos.index') }}" class="btn-primary">
                    <i class="bi bi-plus-lg"></i> Nuevo Movimiento
                </a>
                <a href="{{ route('inventario.activos.alertas') }}" class="btn-alert">
                    <i class="bi bi-bell-fill"></i> Alertas
                </a>
            </div>
        </div>

        <div class="stats-grid">
            <div class="stat-card">
                <span class="stat-label">Total Equipos</span>
                <span class="stat-num">{{ $totalActivos }}</span>
                <i class="bi bi-pc-display stat-icon"></i>
            </div>
            <div class="stat-card">
                <span class="stat-label">Valor Inventario</span>
                <span class="stat-num">${{ number_format($valorInventario, 2) }}</span> 
                <i class="bi bi-cash-stack stat-icon"></i>
            </div>
            <div class="stat-card">
                <span class="stat-label">Asignados</span>
                <span class="stat-num">{{ $porcentajeAsignados }}%</span>
                <i class="bi bi-person-check stat-icon"></i>
            </div>
            <div class="stat-card">
                <span class="stat-label">En Reparación</span>
                <span class="stat-num" style="color: #d97706;">{{ $enReparacion }}</span>
                <i class="bi bi-tools stat-icon"></i>
            </div>
        </div>

        <div class="dashboard-columns">
            
            <div class="card-stack">
                <div class="card-header">
                    <div class="card-title">
                        <div class="icon-sq"><i class="bi bi-pie-chart-fill"></i></div>
                        <span>Distribución de Estados</span>
                    </div>
                </div>
                <div class="chart-container">
                    <canvas id="activosChart"></canvas>
                </div>
            </div>

            <div class="card-stack">
                <div class="card-header">
                    <div class="card-title">
                        <div class="icon-sq"><i class="bi bi-clock-history"></i></div>
                        <span>Movimientos Recientes</span>
                    </div>
                    <a href="{{ route('inventario.movimientos.index') }}" style="font-size: 0.8rem; font-weight: 600; color: #4f46e5; text-decoration: none;">Ver Historial &rarr;</a>
                </div>
                <div style="overflow-x: auto;">
                    <table class="table-min">
                        <thead>
                            <tr>
                                <th>Acta</th>
                                <th>Tipo</th>
                                <th>Responsable</th>
                                <th>Fecha</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($ultimosMovimientos as $mov)
                            <tr>
                                <td style="font-family: monospace; font-weight: 700;">{{ $mov->codigo_acta ?? 'N/A' }}</td>
                                <td>
                                    @if($mov->tipoRegistro)
                                        <span class="badge {{ $mov->tipoRegistro->id == 1 ? 'bg-green' : 'bg-gray' }}">
                                            {{ $mov->tipoRegistro->nombre }}
                                        </span>
                                    @else
                                        <span class="badge bg-gray">Desconocido</span>
                                    @endif
                                </td>
                                <td>{{ $mov->responsable->name ?? 'Sistema' }}</td>
                                <td>{{ $mov->created_at ? $mov->created_at->format('d M, Y') : 'Sin fecha' }}</td>
                            </tr>
                            @empty
                            <tr><td colspan="4" style="text-align:center; padding: 30px; color: #94a3b8;">Sin movimientos registrados</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const ctx = document.getElementById('activosChart').getContext('2d');
            
            const etiquetas = {!! json_encode($labelsGrafica ?? []) !!};
            const valores = {!! json_encode($dataGrafica ?? []) !!};

            const data = {
                labels: etiquetas,
                datasets: [{
                    data: valores,
                    backgroundColor: [
                        '#10b981', '#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#64748b'
                    ],
                    borderWidth: 0,
                    hoverOffset: 4
                }]
            };

            new Chart(ctx, {
                type: 'doughnut',
                data: data,
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    cutout: '75%', 
                    plugins: {
                        legend: {
                            position: 'bottom',
                            labels: {
                                padding: 20,
                                usePointStyle: true,
                                font: { family: "'Inter', sans-serif", size: 12 }
                            }
                        }
                    }
                }
            });
        });
    </script>
</x-base-layout>