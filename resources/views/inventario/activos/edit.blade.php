<x-base-layout>
    ...
    <div class="container">
        ...
        <datalist id="referenciasDataList">
            @foreach($referencias as $ref)
                <option data-id="{{ $ref->id }}" data-pure-name="{{ $ref->referencia }}"
                    data-marca-nom="{{ $ref->marca->nombre ?? 'N/A' }}" data-marca-id="{{ $ref->id_MaeMarcas }}"
                    data-sub-nom="{{ $ref->subgrupo->nombre ?? 'N/A' }}" data-sub-id="{{ $ref->id_MaeSubgrupo }}"
                    data-bodega-nom="{{ $ref->bodega->nombre ?? 'N/A' }}" data-bodega-id="{{ $ref->id_InvBodegas ?? '' }}"
        ...
        <div class="cv-header">
            <div>
                <h1 style="font-size: 1.5rem; font-weight: 900; margin: 0; color: var(--slate-900);">Expediente Técnico
                    del Activo</h1>
                <p style="margin: 5px 0 0 0; color: var(--slate-600); font-weight: 500; font-size: 0.9rem;">Hoja de Vida
                    e Historial de Adquisición</p>
            </div>
            <div style="text-align: right;">
                <div class="asset-badge">MARQUILLA: {{ $activo->codigo_activo }}</div>
                <p style="font-size: 0.75rem; color: var(--slate-500); margin-top: 8px; font-weight: 700;">ID SISTEMA:
                    #{{ $activo->id }}</p>
            </div>
        </div>
        ...
        <form action="{{ route('inventario.activos.update', $activo->id) }}" method="POST"
                    ...
                    <div class="input-grid">
                        <div class="field-group">
                            ...
                            <input type="text" name="codigo_activo"
                                value="{{ old('codigo_activo', $activo->codigo_activo) }}" required>
                        </div>
                        <div class="field-group">
                        ...
                        <div class="field-group">
                            ...
                            <input type="text" name="serial" value="{{ old('serial', $activo->serial) }}"
                                placeholder="N/A">
                        </div>
                ...
                <div class="panel-section">
                    <div class="panel-title" style="justify-content: space-between;">
                        ...
                        <div style="display: flex; gap: 8px;">
                            <button type="button" class="btn-modal-trigger btn-edit-ref" id="btnEditRef">✏️ Editar
                                Referencia</button>
                            <button type="button" class="btn-modal-trigger btn-new-ref" id="btnNewRef">+ Crear
                                Referencia</button>
                        </div>
                    </div>
                    <div class="input-grid">
                        <div class="field-group">
                            ...
                            <input type="hidden" name="invReferencias_id" id="hidden_referencia_id"
                                value="{{ $activo->invReferencias_id }}">
                        </div>
                        <div class="field-group">
                            <label>Marca (Automática)</label>
                            <input type="text" id="display_marca" class="input-inherited"
                                value="{{ $activo->marca->nombre ?? 'N/A' }}" readonly tabindex="-1">
                        </div>
                        <div class="field-group">
                            <label>Subgrupo / Categoría</label>
                            <input type="text" id="display_subgrupo" class="input-inherited"
                                value="{{ $activo->subgrupo->nombre ?? 'N/A' }}" readonly tabindex="-1">
                        </div>
                    </div>
                </div>
                ...
                <div class="panel-section">
                    ...
                    <div class="input-grid">
                        <div class="field-group">
                            ...
                            <input type="date" name="fecha_inicio_garantia"
                                value="{{ old('fecha_inicio_garantia', $activo->fecha_inicio_garantia ? $activo->fecha_inicio_garantia->format('Y-m-d') : '') }}">
                        </div>
                        <div class="field-group">
                            <label>Fin de Garantía</label>
                            <input type="date" name="fecha_fin_garantia"
                                value="{{ old('fecha_fin_garantia', $activo->fecha_fin_garantia ? $activo->fecha_fin_garantia->format('Y-m-d') : '') }}">
                        </div>
                        <div class="field-group">
                            <label>Vida Útil (Meses)</label>
                            <input type="number" name="vida_util_meses"
                                value="{{ old('vida_util_meses', $activo->vida_util_meses) }}">
                        </div>
                        <div class="field-group">
                            <label>Unidad de Medida</label>
                            <input type="text" name="unidad_medida"
                                value="{{ old('unidad_medida', $activo->unidad_medida) }}"
                                placeholder="UND, MTS, PZA...">
                        </div>
                        ...
                        <div class="field-group full-row">
                            ...
                            <div style="display: flex; gap: 8px;">
                                <input type="text" name="hoja_vida" value="{{ old('hoja_vida', $activo->hoja_vida) }}"
                                    placeholder="Ej: https://drive.google.com/..." style="flex: 1;">
                                @if($activo->hoja_vida)
                        ...
                        <div class="field-group">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 4px;">
                                ...
                                @if($activo->id_InvDetalleCompras)
                                    <button type="button" id="btnOpenModalCompra"
                                        class="btn-modal-trigger btn-view-purchase">
                                        Ver Datos de Compra
                                    </button>
                                @endif
                            </div>
                        ...
                        <div class="field-group full-row"
                            style="background: var(--slate-50); padding: 15px; border-radius: 12px; border: 1px dashed var(--slate-300); margin-top: 10px;">
                            <label style="color: var(--slate-900);">Factura</label>

                            @if($activo->detalleCompra && $activo->detalleCompra->id_InvCompras)
                                <div style="margin-bottom: 12px;">
                                    <a href="{{ route('inventario.compras.archivo', $activo->detalleCompra->id_InvCompras) }}"
                            ...
                            <small
                                style="color: var(--slate-500); font-weight: 600; margin-top: 6px; display: block; line-height: 1.4;">
                                Seleccione un archivo solo si desea actualizar el documento en la tabla
                                <b>inv_compras</b>.
                            </small>
                        </div>
                ...
                <div class="panel-section">
                    ...
                    <div class="input-grid">
                        ...
                        <div class="field-group">
                            ...
                            <input type="hidden" name="id_MaeMunicipios" id="hidden_municipio_id"
                                value="{{ $activo->id_MaeMunicipios }}">
                        </div>
                        ...
                        <div class="field-group">
                            ...
                            <input type="hidden" name="id_ultimo_usuario_asignado" id="hidden_usuario_id"
                                value="{{ $activo->id_ultimo_usuario_asignado }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="panel-section" style="margin-bottom: 0; background: var(--slate-50);">
                    ...
                    <div class="input-grid">
                        <div class="field-group">
                            ...
                            <input type="text" class="input-inherited"
                                value="{{ $activo->usuarioRegistro->name ?? 'Sistema' }}" readonly>
                            <input type="hidden" name="id_usersRegistro" value="{{ $activo->id_usersRegistro }}">
                        </div>
                        <div class="field-group">
                            <label>Fecha Creación</label>
                            <input type="text" class="input-inherited"
                                value="{{ $activo->created_at->format('d/m/Y H:i') }}" readonly>
                        </div>
                        <div class="field-group">
                            <label>Última Modificación</label>
                            <input type="text" class="input-inherited"
                                value="{{ $activo->updated_at->format('d/m/Y H:i') }}" readonly>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="action-bar">
                <a href="{{ route('inventario.activos.show', $activo->id) }}"
                    style="color: white; text-decoration: none; font-weight: 600; font-size: 0.9rem;">← Volver al
                    Expediente</a>
    ...
    <div id="modalRef" class="modal-overlay" style="display: none;">
        <div class="modal-content">
            <h3 id="modalTitle" style="margin-top:0; color: var(--slate-900);">Editar Referencia Actual</h3>
            ...
            <div style="margin-top: 25px; display: flex; justify-content: flex-end; gap: 10px;">
                <button type="button" id="btnCloseModal"
                    style="background: var(--slate-100); color: var(--slate-700); font-weight: 700; border: none; padding: 12px 20px; border-radius: 10px; cursor: pointer;">Cancelar</button>
                <button type="button" id="btnSaveRefAjax"
                    style="background: var(--primary); color: white; font-weight: 700; border: none; padding: 12px 20px; border-radius: 10px; cursor: pointer;">Guardar</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <table class="mini-table">
                <tr>
                    ...
                    <td class="value" id="view_compra_id"></td>
                </tr>
                <tr>
                    <td class="label">Cantidades:</td>
                    <td class="value" id="view_compra_cant"></td>
                </tr>
                <tr>
                    <td class="label">Precio Unit:</td>
                    <td class="value" id="view_compra_precio" style="color: var(--success);"></td>
                </tr>
                <tr>
                    <td class="label">Total Item:</td>
                    <td class="value" id="view_compra_subtotal"></td>
                </tr>
                <tr>
                    <td class="label">Referencia:</td>
                    <td class="value" id="view_compra_ref"></td>
                </tr>
                <tr>
                    <td class="label" style="vertical-align: top;">Detalle:</td>
                    <td class="value" id="view_compra_detalle"></td>
                </tr>
            </table>
            ...
            <div style="margin-top: 25px;">
                <button type="button" id="btnCloseModalCompra"
                    style="width: 100%; background: var(--slate-900); color: white; border: none; padding: 14px; border-radius: 12px; cursor: pointer; font-weight: 700;">Cerrar
                    Consulta</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $('#btnSaveRefAjax').click(function () {
            ...
            let data = {
                ...
                referencia: valRef,
                detalle: valDet,
                id_InvMarcas: valMarca,
                id_InvSubGrupos: valSubgrupo,
