        ...
        <div class="card-body">
            ...
            <div class="mb-4 px-4 d-flex align-items-center justify-content-between flex-wrap gap-2">
                <h5 class="fw-bold mb-0 text-primary">
                    <i class="feather-check-circle me-2"></i> Resultados de Interacción
                </h5>
                <a href="{{ route('interactions.outcomes.create') }}" class="btn btn-success d-flex align-items-center gap-2">
            ...
            {{-- 📌 Buscador --}}
            <div class="px-4 pb-4">
                <form action="{{ route('interactions.outcomes.index') }}" method="GET">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center align-items-center gap-1">
                                        {{-- Ver --}}
                                        <a href="{{ route('interactions.outcomes.show', $outcome->id) }}"
                                           class="btn btn-sm btn-light border" data-bs-toggle="tooltip" title="Ver">
                                            <i class="feather-eye"></i>
                                        </a>
                                        {{-- Editar --}}
                                        <a href="{{ route('interactions.outcomes.edit', $outcome->id) }}"
                                           class="btn btn-sm btn-light border" data-bs-toggle="tooltip" title="Editar">
                                            <i class="feather-edit-3"></i>
                                        </a>
                                        {{-- Eliminar --}}
                                        <form action="{{ route('interactions.outcomes.destroy', $outcome->id) }}"
                                              method="POST" class="formEliminar d-inline">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center text-muted py-4">
                                    <i class="feather-info me-1"></i> No hay resultados disponibles
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if ($outcomes->hasPages())
                <div class="d-flex justify-content-center mt-4">
                    {{ $outcomes->appends(request()->except('page'))->links() }}
                </div>
            @endif
        </div>
    </div>
