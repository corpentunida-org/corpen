...
<form id="interaction-form" method="POST"
    ...
    <div
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <p class="text-muted small mb-0"><i class="bi bi-clock me-1"></i> Sesión iniciada • Completa la gestión
                </p>
            </div>
        </div>
    ...
    <div class="tab-content position-relative px-4">
            ...
            <div class="category-container mb-4 p-4">
                <div class="row align-items-start g-4">
                    <div class="col-md-8">
                        ...
                        <div id="error-client-msg" class="text-danger small mt-2" style="display:none;">Debes
                            seleccionar un cliente.</div>
                    </div>
                            ...
                            <div class="d-flex align-items-center flex-grow-1">
                                ...
                                <div class="d-flex gap-1 ms-2">
                                    <button type="button" class="btn btn-sm btn-light border p-1"
                                        id="btn-timer-toggle" title="Pausar/Reanudar"><i class="bi bi-pause"
                                            style="font-size: 0.8rem;"></i></button>
                                    <button type="button" class="btn btn-sm btn-light border p-1"
                                        id="btn-timer-reset" title="Reiniciar"><i
                    ...
                    <div id="history-content" class="position-relative bg-white">
                        <div>
                            <div id="interaction-history-list" class="history-list"
                                ...
                                <div id="history-loading-skeleton" class="py-3 px-2 text-muted small">Cargando
                                    historial...</div>
                ...
                <div class="collapse show" id="paramCollapse">
                    <div class="category-content p-4">
                        ...
                        <div class="mb-4" id="container-channel">
                            <label class="form-label text-uppercase text-muted mb-2 d-block">Canal de Entrada <span
                                    class="text-muted">*</span></label>
                        ...
                        <div class="mb-2" id="container-type">
                            <label class="form-label text-uppercase text-muted mb-2 d-block">Motivo / Tipificación
                        ...
                        <div class="d-flex gap-3 mb-4">
                            <div>
                                <input type="radio" class="btn-check" name="handled_by_agent" id="handled_by_me"
                                    value="yes" checked>
                                <label class="btn btn-outline-dark px-4 py-2" for="handled_by_me">Yo me
                                    encargo</label>
                            </div>
                            <div>
                                <input type="radio" class="btn-check" name="handled_by_agent"
                                    id="handled_by_other" value="no">
                                <label class="btn btn-outline-secondary px-4 py-2" for="handled_by_other">Delegar a
                                    otro</label>
                            </div>
                        </div>
                                ...
                                <select class="form-select" id="id_user_asignacion">
                                    ...
                                    @if (isset($interaction) && $interaction->usuarioAsignado)
                                        <option value="{{ $interaction->id_user_asignacion }}" selected>
                                            {{ $interaction->usuarioAsignado->name }}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
        ...
        <div class="tab-pane fade" id="resultado-tab" role="tabpanel">
            ...
            <div class="category-container mb-4">
                <div class="category-header p-3 bg-light d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="refresh-history"
                        title="Recargar lista">Actualizar</button>
                </div>
                        ...
                        <div class="p-3 bg-white border-bottom">
                            <div id="selected-parent-info"
                                ...
                                <button type="button" class="btn btn-sm btn-light border text-danger"
                                    id="clear-parent-selection">Desvincular</button>
                            </div>
                        </div>
                                    ...
                                    @foreach ($outcomes as $outcome)
                                        <div class="col-sm-6">
                                            ...
                                            <label class="btn btn-outline-secondary w-100 text-start "
                                                for="outcome_{{ $outcome->id }}">{{ $outcome->name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                @error('outcome')
                                        ...
                                        @foreach ($nextActions as $action)
                                            <div class="col-6 col-sm-4">
                                                ...
                                                <label class="btn btn-outline-dark w-100 border small text-truncate"
                                                    ...
                                                    title="{{ $action->name }}">{{ $action->name }}</label>
                                            </div>
                                        @endforeach
                                    </div>
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <label class="form-label text-muted d-block">Notas Finales <span
                            class="text-muted">*</span></label>
            ...
            <div class="d-flex justify-content-between mb-4">
                ...
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-light border text-danger px-3 btn-limpiar-borrador"><i
                            class="bi bi-trash me-1"></i> Borrar Borrador</button>
                    <button type="submit" id="btn-submit-interaccion"
                        class="btn btn-dark px-4">{{ $modoEdicion ? 'Actualizar' : 'Guardar' }}</button>
                </div>
            </div>
                ...
                <div class="d-flex align-items-center justify-content-between w-100">
                    <div class="d-flex align-items-center">
                        <div class="symbol symbol-45px me-3">
                            <div class="symbol-label bg-white bg-opacity-25"><i
                                    class="fas fa-file-invoice-dollar fs-2x text-white"></i></div>
                        </div>
                        <div>
            ...
            <div class="modal-body p-8">
                <form id="formModalComprobante" action="{{ route('cartera.comprobantes.store') }}" method="POST"
                    ...
                    <div class="row g-5">
                        ...
                        <div class="col-md-6">
                            <div class="fv-row mb-4">
                                <label class="form-label fw-bold text-gray-700 fs-7 text-uppercase">Fecha y Hora de
                                    Pago *</label>
                                ...
                                <div class="list-group position-absolute w-100 shadow-lg d-none custom-dropdown-list"
                                    ...
                                    @if (isset($idBanco))
                                        @foreach ($idBanco as $idb)
                                            <button type="button"
                                                class="list-group-item list-group-item-action py-2 px-4 fs-8 border-0 border-bottom"
                                                data-id="{{ $idb->id }}">{{ $idb->numero_cuenta }} -
                                                {{ $idb->banco }}</button>
                                        @endforeach
                                    @endif
                                </div>
...
<div class="modal fade" id="modalIdentificacionBancos" tabindex="-1" data-bs-backdrop="static" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg"
            style="border-radius: 12px; overflow: hidden; background-color: #f8f9fa;">

            <div
                class="modal-header border-bottom border-gray-200 py-3 px-5 bg-white d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <div class="symbol symbol-35px me-3">
                        <div class="symbol-label bg-light-primary text-primary rounded-circle"><i
                                class="fas fa-search-dollar fs-5"></i></div>
                    </div>
                    <div>
            ...
            <div class="bg-white px-5 py-4 border-bottom border-gray-200 shadow-sm">
                <div class="d-flex gap-2">
                    <div class="input-group input-group-solid">
                        ...
                        <input type="text" id="modalBuscadorGlobal" class="form-control ps-0 fs-6 fw-bold"
                            placeholder="Escribe Cédula, Nombre del Cliente, Valor o Hash...">
                    </div>
                    <button class="btn btn-primary fw-bolder px-8" type="button" id="btnBuscarModal">BUSCAR</button>
                </div>
                <div class="mt-2 ps-1">
                    <span class="text-muted fs-10 text-uppercase fw-bolder">Sugerencia: Puedes buscar por valores
                        exactos sin puntos ni comas para mayor precisión.</span>
                </div>
            </div>
            ...
            <div class="modal-body p-3">
                <div class="row g-2">
                    <div class="col-lg-5">
                        <div class="bg-white shadow-sm border border-gray-300 rounded-3 overflow-hidden d-flex flex-column"
                            style="height: 55vh;">
                            <div class="bg-gray-100 border-bottom border-gray-300 px-3 py-2">
                                <span class="fw-bold text-primary fs-9 text-uppercase"><i
                                        class="fas fa-university me-1"></i> Extractos Bancarios</span>
                            </div>
                                    ...
                                    <tbody id="tbodyBancosModal">
                                        <tr class="empty-placeholder">
                                            <td colspan="4" class="text-center py-10 text-muted">
                                                <i class="fas fa-arrow-up d-block fs-1 opacity-25 mb-2"></i> Escribe
                                                algo arriba para buscar
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                    ...
                    <div class="col-lg-7">
                        <div class="bg-white shadow-sm border border-gray-300 rounded-3 overflow-hidden d-flex flex-column"
                            style="height: 55vh;">
                            <div class="bg-gray-100 border-bottom border-gray-300 px-3 py-2">
                                <span class="fw-bold text-info fs-9 text-uppercase"><i
                                        class="fas fa-receipt me-1"></i> Pagos en Cartera</span>
                            </div>
                            <div class="table-responsive flex-grow-1">
                                <table class="table-gsheets table-hover m-0">
                                    <thead class="sticky-top bg-white">
                                    ...
                                    <tbody id="tbodyCarteraModal">
                                        <tr class="empty-placeholder">
                                            <td colspan="5" class="text-center py-10 text-muted">
                                                <i class="fas fa-search d-block fs-1 opacity-25 mb-2"></i> Resultados
                                                de cartera aparecerán aquí
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
            ...
            <div class="modal-footer bg-light py-2 px-5 border-0">
                <button type="button" class="btn btn-sm btn-secondary fw-bold rounded-pill"
...
<form id="formConciliacionModal" action="{{ route('contabilidad.extractos.conciliacion-manual') }}" method="POST"
    style="display: none;">
    @csrf
    <input type="hidden" name="id_interaccion" value="{{ $interaction->id ?? 0 }}">
...
<script>
    ...
    const CRMApp = (function($) {
        ...
        const UI = {
            ...
            showTab: function(tabId) {
                let tabElement = (tabId === 'resultado-tab') ? document.getElementById('profile-tab') :
        ...
        function bindEvents() {
            ...
            $('input[name="outcome"]').on('change', function() {
                ...
                const reqPlan = $(this).data('requires-planning') === true || $(this).data(
                    'requires-planning') === 'true';
                if (reqPlan || val.includes('pendiente') || val.includes('no contesta') || val.includes(
                        'seguimiento')) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function buscarGlobal() {
            ...
            fetch(`{{ route('contabilidad.extractos.buscar-modal') }}?search=${valor}`)
                ...
                .then(data => {
                    ...
                    tbodyBanco.innerHTML = arrayBancos.length ? '' :
                        '<tr><td colspan="4" class="text-center py-5">No hay resultados en bancos</td></tr>';
                    ...
                    tbodyCartera.innerHTML = arrayCartera.length ? '' :
                        '<tr><td colspan="5" class="text-center py-5">No hay resultados en cartera</td></tr>';
                    arrayCartera.forEach((c, i) => {
                        let archivoIcon = c.url_archivo ?
                            `<a href="${c.url_archivo}" target="_blank" class="ms-2 text-info"><i class="fas fa-file-pdf"></i></a>` :
                            '';
        ...
        $('#tbodyBancosModal').on('click', '.item-extracto', function() {
            ...
            if (idx > -1) selectedModalIds.splice(idx, 1);
            else selectedModalIds.push(id);
        });

        $('#tbodyCarteraModal').on('click', '.btn-cruce', function() {
            const idComp = $(this).data('id');
            if (!selectedModalIds.length) return Swal.fire({
                icon: 'warning',
                title: 'Atención',
                text: 'Selecciona movimientos.',
        ...
        $('#modalIdentificacionBancos').on('hidden.bs.modal', function() {
            ...
            if (tbBancos) tbBancos.innerHTML =
                '<tr class="empty-placeholder"><td colspan="4" class="text-center py-10 text-muted">Escribe algo arriba para buscar</td></tr>';
            if (tbCartera) tbCartera.innerHTML =
                '<tr class="empty-placeholder"><td colspan="5" class="text-center py-10 text-muted">Resultados aparecerán aquí</td></tr>';
        });
        ...
        if (formBancos) {
            formBancos.addEventListener('submit', function(e) {
                ...
                fetch(this.action, {
                    ...
                    .then(data => {
                        const modalInst = bootstrap.Modal.getInstance(document.getElementById(
                            'modalIdentificacionBancos'));
                        if (modalInst) modalInst.hide();
                        formBancos.reset();
                        Swal.fire({
                            title: '¡Conciliado!',
                            text: data.message ||
                                'La transacción se ha vinculado correctamente.',
                            icon: 'success',
        ...
        modalElement.querySelectorAll('[data-bs-dismiss="modal"]').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                document.querySelectorAll('.type-trigger').forEach(radio => radio.checked =
                    false);
                document.querySelectorAll('#container-list-types label').forEach(tag => tag
        ...
        if (idObligacionInput) {
            idObligacionInput.addEventListener('change', function() {
                ...
                if (idLineaModal === "5" && ccoInput) {
                    ...
                    if (!ccoLabel.innerHTML.includes('*')) ccoLabel.innerHTML =
                        'CCO <span class="text-danger fw-bolder">*</span>';
                } else if (ccoInput) {
                    ccoInput.removeAttribute('required');
                ...
                if (lineasRequeridas.includes(idLineaModal)) {
                    ...
                    if (prInput) {
                        ...
                        if (!prLabel.innerHTML.includes('*')) prLabel.innerHTML =
                            'PR <span class="text-danger fw-bolder">*</span>';
                    }
                } else {
                    if (cuotaInput) {
                        cuotaInput.removeAttribute('required');
        ...
        if (displayInput) {
            displayInput.addEventListener('input', function() {
                ...
                if (parts[1] !== undefined && parts[1].length > 2) val = parts[0] + '.' + parts[1]
                    .substring(0, 2);
        ...
        if (formComprobante) {
            formComprobante.addEventListener('submit', function(e) {
                e.preventDefault();
                if (!document.getElementById('id_obligacion').value || !document.getElementById(
                        'id_banco').value) {
                ...
                .then(async res => {
                    if (!res.ok) {
                        if (res.status === 419) throw new Error(
                            'La sesión ha caducado por inactividad. Por favor, recarga.');
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            ...
                            if (r.isConfirmed) ejecutarEnvioComprobante(true);
                        });
                    } else {
                        Swal.fire('Error', data.message, 'error');
                    }
                })
                .catch(error => Swal.fire('Error', error.message !== 'Failed to fetch' ? error.message :
        ...
        document.addEventListener('paste', function(e) {
            ...
            if (inputDocumento) {
                ...
                if (typeof toastr !== 'undefined') toastr.success(
                    "¡Archivo adjuntado desde el portapapeles!");
            }
        });
        ...
        document.querySelectorAll('.type-trigger').forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                if (this.value == '3' || labelText === 'soporte de pago') {
                    if (!cID) {
                        Swal.fire('Error', 'Selecciona un cliente.', 'warning');
                        this.checked = false;
                        return;
                    }
                    const modalCodTercero = document.getElementById('cod_tercero');
                    ...
                    const modalElement = document.getElementById('modalComprobante');
                    if (modalElement) {
                        let modalInst = bootstrap.Modal.getInstance(modalElement) ||
                ...
                if (this.value == '20' || labelText === 'identificación bancos' || labelText ===
                    ...
                    const modalBancosElement = document.getElementById(
                        'modalIdentificacionBancos');
                    if (modalBancosElement) {
                        let modalBancos = bootstrap.Modal.getInstance(modalBancosElement) ||
                            new bootstrap.Modal(modalBancosElement);
                        modalBancos.show();
                    }
                }
            });
