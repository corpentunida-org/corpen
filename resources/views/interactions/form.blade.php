<style>
    ...
    .tab-navigation {
        ...
        overflow-x: auto;
        /* Permite scroll horizontal en móviles pequeños */
    ...
    .category-icon {
        ...
        /* Fondo suave en vez de sólido fuerte */
        color: var(--primary-color);
    ...
    .form-select {
        ...
        transition: var(--transition);
        font-size: 16px;
        /* CRÍTICO: 16px previene zoom en iOS */
        width: 100%;
        background-color: var(--card-background);
        color: var(--text-primary);
        min-height: 48px;
        /* Altura mínima táctil accesible */
    }
    ...
    .btn-check:checked+.visual-card .check-badge {
        opacity: 1;
        transform: scale(1);
    }
    ...
    .btn-check:checked+.smart-tag .tag-icon {
        width: 20px;
        opacity: 1;
        margin-right: 8px;
    }
    ...
    .accordion-button-custom[aria-expanded="true"] .accordion-arrow {
    ...
    .form-actions {
        ...
        background: #fff;
        /* Asegura legibilidad sobre fondo */
    }
    ...
    @media (max-width: 768px) {
        ...
        .form-actions {
            ...
            /* Guardar arriba en móvil es más fácil */
            gap: 1.5rem;
        }
...
<div class="card-header">
    <div>
        <h5 class="card-title">{{ $modoEdicion ? 'Editar Interacción' : 'Registro de Seguimiento Diario' }}</h5>
    </div>
    @if ($modoEdicion && $interaction->id)
        <a href="{{ route('interactions.show', $interaction->id) }}" class="btn btn-primary"><i class="bi bi-eye me-1"></i>
            Ver Detalles</a>
    @endif
</div>
        ...
        <div class="tab-panel active" id="principal-tab">
            <div class="category-container mb-4">
                <div class="category-header pb-2 d-flex justify-content-between align-items-center accordion-button-custom"
                    data-bs-toggle="collapse" data-bs-target="#originCollapse" aria-expanded="false"
                ...
                <div class="collapse" id="originCollapse">
                    <div class="category-content pt-4">
                        <label class="form-label small fw-bold text-muted text-uppercase mb-3">¿Quién inicia la
                            interacción?</label>
                        <div class="row g-3">
                            <div class="col-md-6">
                                <input type="radio" class="btn-check" name="caller_type" id="caller_client"
                                    value="client" checked>
                                <label class="visual-card flex-row justify-content-start p-3 gap-3" for="caller_client"
                                    ...
                                    <i class="bi bi-check-circle-fill check-badge fs-5"
                            ...
                            <div class="col-md-6">
                                <input type="radio" class="btn-check" name="caller_type" id="caller_third"
                                    value="third_party">
                                <label class="visual-card flex-row justify-content-start p-3 gap-3" for="caller_third"
                                    style="min-height: auto;">
                        ...
                        <div id="third-party-fields" class="mt-3 p-4 bg-light rounded-3 border border-dashed"
                            style="display: none;">
                            <h6 class="text-primary fw-bold small text-uppercase mb-3"><i
                                    class="bi bi-pencil-square me-2"></i>Datos de quien llama</h6>
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <label class="form-label small fw-bold">Nombre Completo <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="nombre_quien_llama"
                                        name="nombre_quien_llama">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label small fw-bold">Identificación <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="cedula_quien_llama"
                                        name="cedula_quien_llama">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label small fw-bold">Teléfono <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="celular_quien_llama"
                                        name="celular_quien_llama">
                                </div>
                                <div class="col-12 pt-2" id="container-parentezco">
                                    <label class="form-label small fw-bold mb-2">Relación con el titular: <span
                                            class="text-danger">*</span></label>
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach (['familiar' => 'Familiar', 'amigo' => 'Amigo', 'representante' => 'Representante', 'otro' => 'Otro'] as $v => $l)
                                            <input type="radio" class="btn-check" name="parentezco_quien_llama"
                                                id="rel_{{ $v }}" value="{{ $v }}">
                                            <label class="smart-tag py-1 px-3 small" style="font-size: 0.8rem;"
                                                for="rel_{{ $v }}">{{ $l }}</label>
                                        @endforeach
                                    </div>
                                </div>
            ...
            <div class="category-container mb-4">
                <div class="category-content">
                    <div class="row align-items-end g-3">
                        <div class="col-md-8">
                            <label for="client_id"
                                class="form-label required-field text-uppercase small fw-bold text-muted">Buscar
                                ...
                                <select class="form-select select2" id="client_id" name="client_id" required>
                                    ...
                                    @if ($modoEdicion && $interaction->client_id)
                                        <option value="{{ $interaction->client_id }}" selected>
                                            {{ $interaction->client->nom_ter }}</option>
                                    @endif
                                </select>
                            </div>
                            <div id="error-client-msg" class="text-danger small mt-1" style="display:none;">
                                Debes seleccionar un cliente.</div>
                        </div>

                        <div class="col-md-4">
                            <div
                                class="bg-white border rounded p-2 d-flex align-items-center justify-content-between shadow-sm">
                                ...
                                <div class="text-end">
                                    <div class="d-flex align-items-center justify-content-end">
                                        ...
                                        <div class="fw-bold text-primary font-monospace fs-5" id="timer">
                                            00:00</div>
                                    </div>
                        ...
                        <div id="client-info-card" class="mt-4" style="display:none;">
                            <div class="card border-0 shadow-sm"
                                style="border-radius: 12px; border-left: 5px solid var(--crm-primary) !important;">
                                <div class="card-body p-4">
                                    <div class="row" id="client-info-content">
                                        <div class="col-md-5 border-end">
                                            <div class="d-flex align-items-center mb-4">
                                                <div class="client-avatar me-3 shadow-sm" id="info-avatar"
                                                    style="width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg, var(--crm-primary), #818cf8);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:1.5rem;">
                                                    -</div>
                                                <div>
                                                    <h5 class="fw-bold text-dark mb-0" id="info-nombre">Nombre
                                                        Cliente</h5>
                                                    <div class="text-muted small">ID: <span id="info-id"
                                                            class="fw-medium text-dark">-</span></div>
                                                </div>
                                            </div>
                                            <div class="d-flex gap-2 mb-3">
                                                <span class="badge bg-light text-dark border px-3 py-2 rounded-pill"><i
                                                        class="bi bi-tag me-1"></i> <span
                                                        id="info-categoria">Cat</span></span>
                                                <span class="badge bg-light text-dark border px-3 py-2 rounded-pill"><i
                                                        class="bi bi-geo-alt me-1"></i> <span
                                            ...
                                            <div class="row g-3">
                                                <div class="col-6">
                                                    ...
                                                    <span class="text-dark d-flex align-items-center"><i
                                                            class="bi bi-envelope text-primary me-2"></i> <span
                                                            id="info-email" class="text-truncate">...</span></span>
                                                </div>
                                                <div class="col-6">
                                                    <label class="small text-muted fw-bold d-block">Teléfono</label>
                                                    <span class="text-dark d-flex align-items-center"><i
                                                            class="bi bi-telephone text-primary me-2"></i>
                                                        <span id="info-telefono">...</span></span>
                                                </div>
                                                <div class="col-12">
                                                    <label class="small text-muted fw-bold d-block">Dirección</label>
                                                    <span class="text-dark d-flex align-items-center"><i
                                                            class="bi bi-geo text-primary me-2"></i> <span
                                                            id="info-direccion">...</span></span>
                                                </div>
                                            </div>
                                            <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                                                <a id="btn-editar-cliente" href="#" target="_blank"
                                                    class="btn btn-sm btn-light border fw-medium"><i
                                                        class="bi bi-pencil me-1"></i> Editar</a>
                                                <a id="btn-ver-cliente" href="#" target="_blank"
                                                    class="btn btn-sm btn-outline-primary fw-medium"><i
                                                        class="bi bi-eye me-1"></i> Ver Ficha</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="category-content pt-4">
                        <div id="history-content" class="position-relative">
                            <div class="timeline-container">
                                <div id="interaction-history-list" class="history-list custom-scroll"
                                    style="max-height: 600px; overflow-y: auto; padding-right: 15px; min-height: 150px;">
                                    ...
                                    <div id="history-empty-state" class="text-center py-5 d-none">
                                        ...
                                        <p class="text-muted small mb-0">No se encontraron interacciones
                                            previas para este criterio.</p>
                                    </div>
                                </div>
                                <div class="position-absolute bottom-0 start-0 w-100 pe-none"
                                    style="height: 40px; background: linear-gradient(to bottom, rgba(248,249,250,0), rgba(248,249,250,1)); border-bottom-left-radius: 12px; border-bottom-right-radius: 12px;">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="category-container mb-4">
                <div class="category-header pb-2 d-flex justify-content-between align-items-center accordion-button-custom"
                    data-bs-toggle="collapse" data-bs-target="#paramCollapse" aria-expanded="false"
                ...
                <div class="collapse" id="paramCollapse">
                    <div class="category-content pt-4">
                        <div class="mb-5" id="container-channel">
                            <label
                                class="form-label required-field text-uppercase small fw-bold text-muted mb-3 d-block">1.
                                Canal de Entrada <span class="text-danger">*</span></label>
                                ...
                                @foreach ($channels as $channel)
                                    <div class="position-relative">
                                        ...
                                        <label class="visual-card p-2" for="ch_{{ $channel->id }}">
                                            ...
                                            <div class="card-icon">
                                                @if (stripos($channel->name, 'tel') !== false)
                                                    <i class="bi bi-telephone"></i>
                                                @elseif(stripos($channel->name, 'mail') !== false || stripos($channel->name, 'correo') !== false)
                                                    <i class="bi bi-envelope"></i>
                                                @elseif(stripos($channel->name, 'what') !== false)
                                                    <i class="bi bi-whatsapp"></i>
                                                @elseif(stripos($channel->name, 'chat') !== false)
                                                    <i class="bi bi-chat-dots"></i>
                                                @elseif(stripos($channel->name, 'pres') !== false)
                                            ...
                                            <span
                                                class="card-title fw-semibold small text-center lh-sm">{{ $channel->name }}</span>
                                        </label>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="mb-4" id="container-type">
                            <label
                                class="form-label required-field text-uppercase small fw-bold text-muted mb-3 d-block">2.
                                Motivo / Tipificación <span class="text-danger">*</span></label>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <label
                                    class="form-label required-field text-uppercase small fw-bold text-muted mb-2">3.
                                    Notas Finales <span class="text-danger">*</span></label>
                                <textarea class="form-control" name="notes" id="notes" rows="3"
                                    placeholder="Resume los puntos clave..." style="border-left: 4px solid var(--crm-primary);" required>{{ old('notes', $interaction->notes ?? '') }}</textarea>
                                @error('notes')
            ...
            <div class="form-actions sticky-bottom bg-white border-top pt-3 pb-2">
                <div class="text-end w-100">
                    <button type="button" id="btn-siguiente-paso"
                        class="btn btn-primary px-5 rounded-pill shadow fw-bold">
            ...
            <div class="category-container">
                <div class="category-header pb-3" style="background: linear-gradient(to right, #f8f9fa, #fff);">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <h5 class="category-title mb-1 text-primary"><i
                                    class="bi bi-briefcase-fill me-2"></i>Asignación y Contexto</h5>
                            <p class="category-description mb-0 text-muted" style="margin-left: 0;">Define
                                quién se hace cargo y dónde ocurre.</p>
                        </div>
                ...
                <div class="category-content pt-4 px-4 pb-4">
                    ...
                    <div class="mb-4">
                        <label
                            class="form-label text-uppercase small fw-bold text-muted mb-3 d-block text-center">¿Quién
                            es el responsable?</label>

                        <div class="segment-control shadow-sm">
                            <div class="segment-option">
                                <input type="radio" name="handled_by_agent" id="handled_by_me" value="yes"
                            ...
                            <div class="segment-option">
                                <input type="radio" name="handled_by_agent" id="handled_by_other" value="no"
                                    class="segment-input">
                    ...
                    <div class="mb-5">
                        <div id="panel-me">
                            <div class="agent-id-card">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="text-muted small">La interacción quedará registrada bajo tus
                                        credenciales:</div>
                                    <div
                                        class="d-flex gap-2 mt-2 justify-content-center justify-content-sm-start flex-wrap">
                                        <span class="badge bg-white text-secondary border px-3 py-2">
                                            <i class="bi bi-building me-1"></i>
                                            {{ $idAreaAgente ? $areas[$idAreaAgente] ?? 'Área' : 'Sin Área' }}
                                        </span>
                                        <span class="badge bg-white text-secondary border px-3 py-2">
                                            <i class="bi bi-person-badge me-1"></i>
                                            {{ $idCargoAgente ? $cargos[$idCargoAgente] ?? 'Cargo' : 'Sin Cargo' }}
                                        </span>
                                    </div>
                                </div>
                        ...
                        <div id="panel-other" class="delegation-panel" style="display: none;">
                            <h6 class="text-secondary small fw-bold text-uppercase mb-3"><i
                                    class="bi bi-diagram-3 me-2"></i>Selecciona el Destino</h6>
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label for="id_area_de_asignacion" class="form-label small fw-bold">Área
                                        Responsable <span class="text-danger">*</span></label>
                                    <select class="form-select select2" id="id_area_de_asignacion"
                                        name="id_area_de_asignacion">
                                ...
                                <div class="col-md-6">
                                    <label for="id_cargo_asignacion" class="form-label small fw-bold">Cargo
                                        Responsable <span class="text-danger">*</span></label>
                                    <select class="form-select select2" id="id_cargo_asignacion"
                                        name="id_cargo_asignacion">
                    ...
                    <div class="row g-4 mt-2">
                        <div class="col-md-6">
                            <label for="id_linea_de_obligacion"
                                class="form-label required-field text-uppercase small fw-bold text-muted">Línea
                                de Obligación <span class="text-danger">*</span></label>
                            <div class="input-group shadow-sm">
                                <span class="input-group-text bg-white border-end-0 text-secondary"><i
                                        class="bi bi-credit-card-2-front"></i></span>
                                <select class="form-select border-start-0 select2" id="id_linea_de_obligacion"
                                    name="id_linea_de_obligacion">
                        ...
                        <div class="col-md-6">
                            <label for="id_distrito_interaccion"
                                class="form-label text-uppercase small fw-bold text-muted">Distrito / Zona
                                <span class="text-danger">*</span></label>

                            <div class="sync-wrapper shadow-sm" id="wrapper_distrito">
                                <div class="sync-select-container">
                                    <select class="form-select select2" id="id_distrito_interaccion"
                                        name="id_distrito_interaccion">
                                ...
                                <button type="button" class="sync-btn" id="sync-from-client-btn"
                                    ...
                                    title="Traer distrito del Cliente">
                                    <i class="bi bi-cloud-download"></i>
                                </button>
                                <button type="button" class="sync-btn sync-btn-upload" id="sync-to-client-btn"
                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                    title="Guardar este distrito en el Cliente">
                                    <i class="bi bi-cloud-upload"></i>
                                </button>
                            </div>
                            <div class="d-flex justify-content-end mt-1">
                                <span class="badge bg-light text-muted border" style="font-size: 0.65rem;">SYNC
                                    TOOLS</span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="form-actions sticky-bottom bg-white border-top pt-3 pb-2 mt-4">
                <div class="action-buttons w-100 d-flex justify-content-between">
                    <button type="button" class="btn btn-outline-secondary px-4 rounded-pill border-0 hover-shadow"
                        onclick="showTab('principal')">
                        <i class="bi bi-arrow-left me-2"></i> Volver
                    </button>
                    <button type="button" id="btn-paso2-siguiente"
                        class="btn btn-primary px-5 rounded-pill shadow-lg fw-bold">
        ...
        <div class="tab-panel" id="resultado-tab">
            <div class="category-container">
                <div class="d-flex align-items-center mb-4 pb-3 border-bottom">
                    ...
                    <div>
                        ...
                        <p class="text-muted small mb-0">Selecciona el resultado final. Si requiere
                            seguimiento, se activará la agenda.</p>
                    </div>
                </div>
            </div>
            <div class="category-content">
                <div class="row g-4">
                                    ...
                                    <div class="row g-2">
                                        @foreach ($outcomes as $outcome)
                                            <div class="col-6">
                                                ...
                                                <label class="selection-card w-100 shadow-sm"
                                                    for="outcome_{{ $outcome->id }}">
                                                    <div class="check-icon"><i class="bi bi-check"></i>
                                                    </div>
                    ...
                    <div class="col-lg-6" id="planning-section" style="display:none;">
                        <div class="card border border-warning border-opacity-25 shadow-sm h-100 animate__animated animate__fadeIn"
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="d-flex align-items-center mb-4 text-warning">
                                    ...
                                    <h6 class="text-uppercase fw-bold mb-0 text-dark small"
                                        style="letter-spacing: 1px;">Agenda de Seguimiento</h6>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label fw-bold small text-muted text-uppercase mb-2">Siguiente
                                        Paso</label>
                                    <div class="row g-2">
                                        @foreach ($nextActions as $action)
                                            <div class="col-4">
                                                ...
                                                <label class="selection-card selection-card-warning w-100"
                                                    for="action_{{ $action->id }}">
                                                    <div class="check-icon"><i class="bi bi-check"></i>
                                                    </div>
                                                    <span class="small"
                                                        style="font-size: 0.75rem;">{{ $action->name }}</span>
                                                </label>
                                            </div>
                                        @endforeach
                                    </div>
                                ...
                                <div class="mb-3">
                                    <label for="next_action_date"
                                        class="form-label fw-bold small text-muted text-uppercase">Fecha
                                        Programada</label>
                                    ...
                                    <div class="mt-2 btn-group w-100" role="group">
                                        <button type="button" class="btn btn-sm btn-outline-warning"
                                            onclick="addDays(1)">+1 Día</button>
                                        <button type="button" class="btn btn-sm btn-outline-warning"
                                            onclick="addDays(3)">+3 Días</button>
                                        <button type="button" class="btn btn-sm btn-outline-warning"
                                            onclick="addDays(7)">+1 Sem</button>
                                        <button type="button" class="btn btn-sm btn-outline-warning"
                                            onclick="addMonth(1)">+1 Mes</button>
                                    </div>
                                ...
                                <div class="form-group">
                                    <label for="next_action_notes"
                                        class="form-label fw-bold small text-muted text-uppercase">Instrucciones</label>
                                    <textarea
                                        class="form-control bg-white border-warning border-opacity-25 @error('next_action_notes') is-invalid @enderror"
                                        id="next_action_notes" name="next_action_notes" rows="2" style="resize: none; border-radius: 8px;"
            ...
            <div class="category-container">
                <div class="d-flex align-items-center mb-4 pb-3 border-bottom">
                    ...
                    <div>
                        <h5 class="fw-bold text-dark mb-0" style="font-size: 1.1rem;">Documentación de Soporte
                        </h5>
                        <p class="text-muted small mb-0">Gestiona archivos adjuntos y enlaces externos
                            relacionados con esta gestión.</p>
                    </div>
                </div>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group h-100">
                                <label class="form-label fw-semibold small text-uppercase text-muted mb-2">Archivo
                                    Adjunto</label>
                                ...
                                <div class="drop-zone-pro" id="drop-zone">
                                    ...
                                    <div class="dz-message" id="dz-message-container">
                                        ...
                                        <h6 class="fw-bold text-dark mb-1" id="file-label">Haz clic o arrastra
                                            un archivo aquí</h6>
                                        <p class="text-muted small mb-0">Soporta PDF, Word, Excel, Imágenes
                                            (Max 10MB)</p>
                                    </div>
                                </div>
                                ...
                                @if ($modoEdicion && $interaction->attachment_urls)
                                    <div class="mt-3">
                                        <label
                                            class="form-label fw-semibold small text-uppercase text-muted mb-2">Archivo
                                            Actual</label>
                                        <div class="attachment-card">
                                            ...
                                            <div class="flex-grow-1 overflow-hidden me-3">
                                                <div class="fw-semibold text-dark text-truncate small"
                                                    title="{{ basename($interaction->attachment_urls) }}">
                                                    {{ basename($interaction->attachment_urls) }}
                                                </div>
                                                <div class="text-muted" style="font-size: 0.75rem;">Almacenado
                                                    en servidor</div>
                                            </div>
                                ...
                                <div id="image-preview-wrapper" class="mt-3" style="display: none;">
                                    <div class="p-2 border rounded bg-white shadow-sm d-inline-block">
                                        <img id="img-preview-element" class="img-fluid rounded"
                        ...
                        <div class="col-lg-6">
                            <div class="form-group h-100">
                                <label class="form-label fw-semibold small text-uppercase text-muted mb-2">Referencia
                                    Externa</label>

                                <div class="url-card d-flex flex-column justify-content-center">
                                    <div class="mb-3">
                                        <label for="interaction_url" class="small text-muted mb-1">Enlace /
                                    ...
                                    <div class="d-flex align-items-start p-3 rounded bg-primary shadow-sm">
                                        ...
                                        <p class="small mb-0 lh-sm text-white">
                                            Utiliza este campo para vincular carpetas de Drive, grabaciones de
                                            llamadas, tickets externos o cualquier recurso web que no requiera
                                            descarga.
                                        </p>
                                    </div>
                                </div>
                ...
                <div class="category-container">
                    <div class="category-header d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="category-description mb-0">Consulta la línea de tiempo. Selecciona una
                                interacción para escalar.</p>
                        </div>

                        <div>
                            <button type="button" class="btn btn-sm btn-light border shadow-sm text-primary"
                                id="refresh-history" title="Recargar lista">
                    ...
                    <div class="category-content bg-light p-0">
                        <div id="no-client-selected" class="text-center py-5">
                            ...
                            <p class="text-muted small">Selecciona un cliente arriba para cargar su línea de
                                tiempo.</p>
                        </div>
                        ...
                        <div id="history-section" style="display:none;">
                            ...
                            <div class="p-3">
                                <div id="selected-parent-info"
                                    class="alert alert-primary shadow-sm border-0 d-flex align-items-center mb-4"
                                    style="display:none; background-color: #cfe2ff;">
                                    <div class="me-3">
                                        <span
                                            class="bg-primary text-white rounded-circle p-2 d-inline-flex align-items-center justify-content-center"
                                            style="width: 40px; height: 40px;">
                                            <i class="bi bi-diagram-3-fill fs-5"></i>
                                        </span>
                                    </div>
                                    <div class="flex-grow-1">
                                        <h6 class="alert-heading fw-bold mb-0 text-primary">Escalando Interacción
                                        </h6>
                                        <div class="small text-dark">
                                            Vinculando nueva gestión a: <strong id="selected-parent-text"
                                                class="bg-white px-2 rounded text-dark border ms-1">...</strong>
                                        </div>
                                    </div>
                                    <button type="button"
                                        class="btn btn-sm btn-outline-primary bg-white text-primary border-0 shadow-sm"
                ...
                <div class="form-actions border-top bg-white p-3 mt-0">
                    <div class="d-flex justify-content-between w-100">
                        ...
                        <div class="action-buttons">
                            ...
                            <button type="submit" id="btn-submit-interaccion"
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            inputsToWatch.forEach(input => {
                if (input) {
                    ...
                    input.addEventListener('change', function() {
                        ...
                        if (this.id === 'id_distrito_interaccion') Visuals.cleanError(document
                            .getElementById('wrapper_distrito'), true);
                    });
            ...
            if (btnNext2) {
                btnNext2.addEventListener('click', function(e) {
                    ...
                    if (!distritoSelect.value) {
                        ...
                        isValid = false;
                        if (!firstErrorElement) firstErrorElement = document.getElementById(
                            'wrapper_distrito');
                    }
            ...
            if (btnSyncFrom) {
                btnSyncFrom.addEventListener('click', async () => {
                    ...
                    if (!clientId) return Swal.fire({
                        icon: 'warning',
                        title: '¡Espera!',
                        text: 'Selecciona un cliente en la pestaña anterior.',
                    ...
                    try {
                        ...
                        setTimeout(() => {
                            ...
                            toast.fire({
                                icon: 'success',
                                title: 'Distrito cargado'
                            });

                            // Limpiamos errores si los había
                            Visuals.cleanError(document.getElementById('wrapper_distrito'),
                                true);
            ...
            if (btnSyncTo) {
                btnSyncTo.addEventListener('click', async () => {
                    ...
                    if (!distId) {
                        ...
                        return Swal.fire({
                            icon: 'warning',
                            text: 'Selecciona un distrito para guardar'
                        });
                    }
            ...
            radioNames.forEach(name => {
                ...
                radios.forEach(r => {
                    r.addEventListener('change', function() {
                        let containerId = '';
                        if (name === 'interaction_channel') containerId =
                            'container-channel';
                        if (name === 'interaction_type') containerId = 'container-type';
                        if (name === 'parentezco_quien_llama') containerId =
                            'container-parentezco';

                        const container = document.getElementById(containerId);
                        if (container) {
                            const innerContainer = container.querySelector(
                                    '.grid-gallery') || container.querySelector(
                                    '.bg-light') ||
                                container;
            ...
            if (DOM.btnNext) {
                DOM.btnNext.addEventListener('click', function(e) {
                    ...
                    if (isThirdParty) {
                        ...
                        const parentesco = document.querySelector(
                            'input[name="parentezco_quien_llama"]:checked');
                        if (!parentesco) {
                            UIManager.markContainerError(DOM.third
                                .containerParent); // Contenedor específico
                            isValid = false;
                    ...
                    if (isValid) {
                        ...
                        if (typeof showTab === 'function') {
                            ...
                            console.log(
                                'Navegando a siguiente tab (función showTab no definida en este snippet)'
                            );
                        }
                    } else {
    ...
    <script>
        $(document).ready(function() {
            ...
            function selectParentInteraction(interactionId, interactionInfo) {
                ...
                console.log('Valor de parent_interaction_id después de establecer:', $('#parent_interaction_id')
                    .val());
            ...
            function renderHistoryCard(item) {
                ...
                const safeNotes = item.notes ? $('<div>').text(item.notes).html() :
                    '<em class="text-muted small">Sin notas registradas.</em>';
                ...
                if (files.length > 0) {
                    attachmentButtonsHtml = files.map((rawFileName) => {
                        ...
                        if (dbFileName.startsWith('/') || dbFileName.startsWith('\\')) dbFileName =
                            dbFileName.substring(1);
            ...
            function refreshClientHistory() {
                ...
                $.ajax({
                    ...
                    if (data.history && data.history.length) {
                        ...
                        if (parentId) {
                            ...
                            if ($selectedItem.length) {
                                ...
                                $selectedItem.find('.card-header').addClass('bg-primary text-white')
                                    .removeClass('bg-white');
                                $selectedItem.find('.text-dark').removeClass('text-dark').addClass(
                                    'text-white');

                                const parentItem = data.history.find(item => item.id == parentId);
                                if (parentItem) {
                                    $('#selected-parent-text').text(
                                        `${parentItem.type} - ${parentItem.date}`);
                    ...
                    $historyList.html(
                        `<div class="alert alert-danger m-3">Error cargando historial: ${error}</div>`);
            ...
            $('#client_id').on('change', function() {
                ...
                $.ajax({
                    ...
                    .fail(function(xhr, status, error) {
                        ...
                        if (xhr.responseJSON && xhr.responseJSON.error) errorMessage = xhr.responseJSON
                            .error;
                        else if (status === 'timeout') errorMessage =
