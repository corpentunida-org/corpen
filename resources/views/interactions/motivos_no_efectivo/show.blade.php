    ...
    <div class="container-fluid py-4 px-lg-5">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-dots mb-2">
                        <li class="breadcrumb-item"><a href="{{ route('interactions.motivos_no_efectivo.index') }}" class="text-muted text-decoration-none">Motivos</a></li>
                        <li class="breadcrumb-item active">Detalle</li>
                    </ol>
                </nav>
                <h2 class="fw-black tracking-tight text-dark mb-1">{{ $motivo->name }}</h2>
                <p class="text-secondary opacity-75">Resumen del motivo y su impacto en las gestiones No Efectivas.</p>
            </div>
            <div class="mt-3 mt-md-0 d-flex gap-2">
                <a href="{{ route('interactions.motivos_no_efectivo.index') }}" class="btn btn-white rounded-pill px-4 shadow-sm border text-decoration-none text-dark">
                    <i class="feather-arrow-left me-1"></i> Volver
                </a>
                <a href="{{ route('interactions.motivos_no_efectivo.edit', $motivo->id) }}" class="btn btn-indigo rounded-pill px-4 shadow-sm d-flex align-items-center gap-2 text-decoration-none">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <div class="p-3 bg-light rounded-3 fw-bold text-dark border-start border-indigo border-3">
                                    <span class="text-indigo me-1">#</span>{{ $motivo->id }}
                                </div>
                            </div>
                            <div class="col-md-6">
                                <label class="text-uppercase fs-xs fw-bold text-muted mb-2 d-block" style="letter-spacing: 1px;">Nombre del Motivo</label>
                                <div class="p-3 bg-light rounded-3 fw-bold text-dark">
                                    <i class="feather-x-circle me-2 text-indigo"></i> {{ $motivo->name }}
                                </div>
                            </div>
                            <div class="col-12">
                                ...
                                <div class="p-3 bg-light rounded-3 text-dark d-flex align-items-center justify-content-between">
                                    <span>
                                        ...
                                        @if($motivo->created_at)
                                            {{ $motivo->created_at->format('d \d\e F, Y \a \l\a\s H:i') }}
                                        @else
                                    ...
                                    @if($motivo->created_at)
                                        <span class="badge bg-white text-indigo border ms-2 rounded-pill shadow-sm px-3">
                                            {{ $motivo->created_at->diffForHumans() }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                ...
                @php $usos = $motivo->seguimientos_count ?? 0; @endphp
                            ...
                            <div>
                                ...
                                @if($usos > 0)
                                    <p class="mb-0 small text-muted">
                                        Este registro está <strong>protegido</strong> porque se encuentra vinculado a <strong>{{ $usos }}</strong> seguimientos. No puede ser eliminado para no romper el historial de gestión.
                                    </p>
                                @else
                                    <p class="mb-3 small text-muted">Este motivo no tiene actividad vinculada y puede ser eliminado de forma segura.</p>
                                    <form action="{{ route('interactions.motivos_no_efectivo.destroy', $motivo->id) }}" method="POST" class="formEliminar">
                                        @csrf @method('DELETE')
            ...
            <div class="col-lg-4">
                <div class="card border-0 shadow-lg rounded-4 bg-indigo text-white overflow-hidden mb-4">
                    <div class="card-body p-4 position-relative">
                        <i class="feather-x-circle position-absolute end-0 bottom-0 mb-n4 me-n3 opacity-25" style="font-size: 8rem;"></i>
                        <h5 class="fw-bold mb-4 opacity-75">Impacto Operativo</h5>
                        <div class="display-3 fw-black mb-1 tracking-tight">{{ $usos }}</div>
                        <p class="fw-bold small mb-0">Gestiones registradas</p>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-white p-4 text-center">
                    ...
                    <h6 class="fw-bold text-dark mb-2">Trazabilidad Completa</h6>
                    <p class="small text-muted mb-0">Los motivos ayudan a entender por qué una gestión no fue efectiva.</p>
                </div>
            </div>
        </div>
    </div>
