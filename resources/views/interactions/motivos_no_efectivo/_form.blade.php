<div class="row g-4 px-md-2">
        ...
        <div class="form-group">
            ...
            <div class="input-group input-group-lg border rounded-3 overflow-hidden custom-input-group @error('name') is-invalid border-danger @enderror">
                ...
                <input type="text"
                       ...
                       value="{{ old('name', $motivo->name ?? '') }}"
                       placeholder="Ej: Número equivocado, Sin tono..."
                       required
            ...
            @else
                <small class="form-text text-muted mt-2 d-block opacity-75">
                    Define un nombre claro para la razón de No Efectivo.
                </small>
            @enderror
        </div>
    </div>

    {{-- Campo: Área (a qué equipo pertenece este motivo) --}}
    <div class="col-12">
        <div class="form-group">
            ...
            @if ($puedeElegirArea ?? false)
                <select name="area" class="form-select form-select-lg border rounded-3">
                    <option value="" @selected(old('area', $motivo->area ?? '') === '')>Compartido (todas las áreas)</option>
                    @foreach ($areas as $a)
                        <option value="{{ $a }}" @selected(old('area', $motivo->area ?? '') === $a)>{{ strtoupper($a) }}</option>
                    @endforeach
                </select>
            ...
            @else
                <div class="alert alert-light border fs-13 mb-0">
                    ...
                    {{ isset($motivo) ? 'Es de tu área (' . strtoupper($motivo->area ?? $miArea ?? '—') . ').' : 'Quedará para tu área (' . strtoupper($miArea ?? '—') . '), solo tu equipo la verá.' }}
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="col-12">
        <div class="d-flex align-items-center justify-content-end gap-3">
            <a href="{{ route('interactions.motivos_no_efectivo.index') }}" class="btn btn-link text-muted text-decoration-none fw-semibold">
                Cancelar
            </a>
            <button type="submit" class="btn btn-indigo btn-lg rounded-pill px-5 shadow-sm d-flex align-items-center gap-2">
                ...
                <span>{{ $buttonText ?? 'Guardar Motivo' }}</span>
            </button>
        </div>
    </div>
</div>
