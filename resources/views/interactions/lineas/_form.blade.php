        ...
        <div class="form-group">
            ...
            <div class="input-group input-group-lg border rounded-3 overflow-hidden custom-input-group @error('name') is-invalid border-danger @enderror">
                ...
                <input type="text"
                       ...
                       value="{{ old('name', $linea->name ?? '') }}"
                       placeholder="Ej: Póliza Vida Deudor, Crédito Libre Inversión..."
                       required
            ...
            @else
                <small class="form-text text-muted mt-2 d-block opacity-75">
                    El producto/línea de obligación que se relaciona con una interacción.
                </small>
            @enderror
        </div>
    </div>

    {{-- Campo: Área (a qué equipo pertenece esta línea) --}}
    <div class="col-12">
        <div class="form-group">
            ...
            @if ($puedeElegirArea ?? false)
                <select name="area" class="form-select form-select-lg border rounded-3">
                    <option value="" @selected(old('area', $linea->area ?? '') === '')>Compartido (todas las áreas)</option>
                    @foreach ($areas as $a)
                        <option value="{{ $a }}" @selected(old('area', $linea->area ?? '') === $a)>{{ strtoupper($a) }}</option>
                    @endforeach
                </select>
            ...
            @else
                <div class="alert alert-light border fs-13 mb-0">
                    ...
                    {{ isset($linea) ? 'Es de tu área (' . strtoupper($linea->area ?? $miArea ?? '—') . ').' : 'Quedará para tu área (' . strtoupper($miArea ?? '—') . '), solo tu equipo la verá.' }}
                </div>
            @endif
        </div>
    </div>
