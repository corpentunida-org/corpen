@php
    ...
    $fechaVal = old(
        'fecha_solicitud',
        $esEdicion ? $correspondencia->fecha_solicitud->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i'),
    );
@endphp
...
<div class="container-fluid py-2">
        ...
        <div class="card-body">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-3">
                    ...
                    @else
                        <select name="medio_recibido" class="form-select" required>
                            ...
                            @foreach ($medios as $m)
                                <option value="{{ $m->id }}"
                                    {{ old('medio_recibido') == $m->id ? 'selected' : '' }}>{{ $m->nombre }}
                                </option>
                            @endforeach
                        </select>
                    @endif
                ...
                <div class="col-md-3">
                    ...
                    @if ($esEdicion)
                        <div class="static-value"><span
                                class="badge bg-secondary text-dark border">{{ $correspondencia->estado->nombre ?? 'N/A' }}</span>
                        </div>
                        <input type="hidden" name="estado_id" value="{{ $correspondencia->estado_id }}">
                    @else
                        <select name="estado_id" class="form-select" required>
                            @foreach ($estados as $e)
                                <option value="{{ $e->id }}" {{ $e->id == 3 ? 'selected' : '' }}>
                                    {{ $e->nombre }}</option>
                            @endforeach
                        </select>
                    @endif
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body">
                    <h6 class="field-label text-primary mb-3 border-bottom pb-2"><i class="bi bi-text-paragraph"></i>
                        Contenido</h6>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        @if ($esEdicion)
                            <div class="p-3 bg-light rounded text-muted small border">
                                {{ $correspondencia->observacion_previa ?? 'Sin observaciones.' }}</div>
                            <input type="hidden" name="observacion_previa"
                                value="{{ $correspondencia->observacion_previa }}">
                        @else
                ...
                <div class="card resolution-card mb-4" id="card_cierre">
                    <div class="card-body p-4">
                        ...
                        <textarea name="final_descripcion" id="final_descripcion" class="form-control" rows="5"
                            placeholder="Gestión final...">{{ old('final_descripcion', $correspondencia->final_descripcion) }}</textarea>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body">
                    <h6 class="field-label text-primary mb-3 border-bottom pb-2"><i class="bi bi-sliders"></i>
                        Clasificación</h6>

                    <div class="mb-4">
                        <label class="field-label">Remitente <span class="text-danger">*</span></label>
                        @if ($esEdicion)
                            <div class="static-value fw-bold">{{ $correspondencia->remitente->cod_ter ?? '' }} -
                                {{ $correspondencia->remitente->nom_ter ?? 'N/A' }}</div>
                            <input type="hidden" name="remitente_id" id="remitente_id"
                                value="{{ $correspondencia->remitente_id }}">
                        @else
                            @candirect('correspondencia.asignar.remitente')
                            <select name="remitente_id" id="remitente_id" class="form-select select2-remitente"
                                required>
                        ...
                        @else
                            <select name="remitente_id" id="remitente_id" class="form-select select2-remitente"
                                required>
                                <option value="{{ auth()->user()->nid }}">{{ auth()->user()->name }}</option>
                            </select>
                    ...
                    <div class="remitente-data-card" id="remitente-data-card" style="display: none;">
                        ...
                        <div class="d-flex justify-content-end mt-3">
                            <button type="button" id="btn-actualizar-tercero" class="btn btn-sm btn-outline-primary"
                                style="display: none;">
                    ...
                    <div class="mb-4 pt-3 border-top d-flex justify-content-between">
                        ...
                        @if ($esEdicion)
                            <span
                                class="badge {{ $correspondencia->es_confidencial ? 'bg-warning text-dark' : 'bg-light border' }}">
                                {{ $correspondencia->es_confidencial ? 'SÍ' : 'NO' }}
                            </span>
                        @else
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="es_confidencial"
                                    value="1" checked>
                            </div>
                        @endif
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        const CLIENT_DATA_URL = "{{ route('correspondencia.ajax.remitente.codigo', 'PLACEHOLDER') }}".replace(
            'PLACEHOLDER', '');
