...
<div class="dropdown nxl-h-item d-none" id="notificationComponent">
    ...
    <div class="nxl-head-link me-3 position-relative" 
        ...
        <span class="badge bg-danger nxl-h-badge pulse-animation" id="contadorNotificaciones" aria-label="Notificaciones no leídas"></span>
        ...
        <div class="notifications-header">
            <div class="d-flex justify-content-between align-items-center notifications-head">
                ...
                <div>
                    ...
                    <button class="btn btn-sm btn-icon me-1" 
                            id="speechToggle" 
                    ...
                    <button class="btn btn-sm btn-icon me-1" 
                            ...
                            aria-label="Activar sonido de notificación"
                            aria-pressed="false">
                        <i class="feather-volume-2" id="soundIcon"></i>
                    </button>
                    <button class="btn btn-sm btn-icon refresh-btn" 
                            id="refreshBtn" 
                            title="Actualizar"
            ...
            <div class="notification-tabs" role="tablist">
                <div class="tab-item active" 
                     data-category="sinAsignar" 
                     role="tab" 
                     aria-selected="true" 
                     aria-controls="listaNotificaciones"
                     tabindex="0">
...
.notification-bell { transition: var(--transition); }
.notification-bell:hover { transform: scale(1.05); color: #073B4C; }
.badge.bg-danger { background: linear-gradient(135deg, #FFD166, #F77F00) !important; animation: pulse 2s infinite; }
.notification-indicator { position: absolute; top: -5px; right: -5px; width: 8px; height: 8px; background: linear-gradient(135deg, #06D6A0, #118AB2); border-radius: 50%; opacity: 0; transform: scale(0); transition: var(--transition); }
.notification-indicator.show { opacity: 1; transform: scale(1); animation: blink 1.5s infinite; }

.notification-panel { width: 380px; max-height: 500px; border-radius: 12px; border: 1px solid var(--border-light); box-shadow: var(--shadow-md); overflow: hidden; animation: slideIn 0.3s ease; }

.notifications-header { padding: 15px; background: linear-gradient(135deg, var(--pastel-gray) 0%, white 100%); border-bottom: 1px solid var(--border-light); }
.notifications-head h6 { font-size: 1rem; color: var(--text-primary); }
.btn-icon { background: none; border: none; padding: 4px; border-radius: 6px; color: var(--text-secondary); transition: var(--transition); }
.btn-icon:hover { background: var(--pastel-gray); color: var(--text-primary); }
.btn-icon.spinning i { animation: spin 1s linear infinite; }

.notification-tabs { display: flex; gap: 8px; margin-top: 12px; overflow-x: auto; padding-bottom: 4px; }
.notification-tabs::-webkit-scrollbar { height: 3px; }
.notification-tabs::-webkit-scrollbar-track { background: var(--border-light); border-radius: 3px; }
.notification-tabs::-webkit-scrollbar-thumb { background: #CED4DA; border-radius: 3px; }
.tab-item { display: flex; flex-direction: column; align-items: center; padding: 8px 10px; border-radius: 10px; background: white; border: 1px solid var(--border-light); cursor: pointer; transition: var(--transition); min-width: 70px; flex-shrink: 0; }
.tab-item:hover { transform: translateY(-2px); box-shadow: var(--shadow-sm); }
.tab-item.active { background: linear-gradient(135deg, var(--pastel-blue), #B6D4FE); border-color: #B6D4FE; transform: translateY(-2px); box-shadow: var(--shadow-sm); }
.tab-icon { font-size: 1.1rem; margin-bottom: 4px; color: var(--text-secondary); transition: var(--transition); }
.tab-item.active .tab-icon { color: var(--text-primary); }
.tab-label { font-size: 0.65rem; font-weight: 500; color: var(--text-secondary); text-align: center; }
.tab-item.active .tab-label { color: var(--text-primary); }
.tab-count { font-size: 0.6rem; font-weight: 600; color: white; background: var(--text-secondary); padding: 1px 4px; border-radius: 8px; margin-top: 2px; min-width: 16px; text-align: center; }
.tab-item.active .tab-count { background: #073B4C; }

.notifications-list { max-height: 320px; overflow-y: auto; padding: 8px; transition: max-height 0.4s ease; }
.notifications-list::-webkit-scrollbar { width: 6px; }
.notifications-list::-webkit-scrollbar-track { background: var(--border-light); border-radius: 3px; }
.notifications-list::-webkit-scrollbar-thumb { background: #CED4DA; border-radius: 3px; }

.notifications-item { margin-bottom: 8px; border-radius: 8px; transition: var(--transition); position: relative; overflow: hidden; }
.notifications-item::before { content: ''; position: absolute; top: 0; left: 0; width: 4px; height: 100%; }
.notifications-item.unassigned::before { background: linear-gradient(135deg, #FFD166, #F77F00); }
.notifications-item.inprogress::before { background: linear-gradient(135deg, #118AB2, #073B4C); }
.notifications-item.review::before { background: linear-gradient(135deg, #7209B7, #560BAD); }
.notifications-item.closed::before { background: linear-gradient(135deg, #06D6A0, #0A9396); }
.notifications-desc { padding: 12px; background: white; border: 1px solid var(--border-light); border-radius: 8px; }
.notification-icon { display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 50%; background: var(--pastel-gray); color: var(--text-secondary); flex-shrink: 0; }
.notifications-item.unassigned .notification-icon { background: var(--pastel-yellow); color: #856404; }
.notifications-item.inprogress .notification-icon { background: var(--pastel-blue); color: #084298; }
.notifications-item.review .notification-icon { background: var(--pastel-purple); color: #4C1D95; }
.notifications-item.closed .notification-icon { background: var(--pastel-green); color: #0F5132; }
.single-task-list-link { text-decoration: none; color: inherit; }
.single-task-list-link:hover { text-decoration: none; color: inherit; }
.notifications-date { margin-top: 8px; padding-top: 8px; font-size: 0.75rem; }

.notifications-footer { padding: 12px; background: var(--pastel-gray); border-top: 1px solid var(--border-light); }
.view-all-link { display: inline-flex; align-items: center; gap: 6px; color: var(--text-primary); text-decoration: none; font-weight: 500; transition: var(--transition); }
.view-all-link:hover { color: #073B4C; gap: 10px; text-decoration: none; }

.empty-icon { font-size: 2rem; margin-bottom: 8px; opacity: 0.7; }

/* MEJORA 15: Estilos para Skeleton Loading */
.skeleton-loader { padding: 8px; }
.skeleton-item { margin-bottom: 8px; border-radius: 8px; padding: 12px; background: white; border: 1px solid var(--border-light); }
...
.skeleton-text { height: 12px; width: 100%; margin-bottom: 4px; }
.skeleton-text:last-child { width: 80%; }

@keyframes loading { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }

/* MEJORA 13: Estilos para Botón de Lectura */
.speak-btn { position: absolute; top: 8px; right: 8px; background: rgba(255, 255, 255, 0.9); border: 1px solid var(--border-light); border-radius: 50%; width: 28px; height: 28px; display: none; align-items: center; justify-content: center; cursor: pointer; color: var(--text-secondary); font-size: 0.9rem; transition: var(--transition); z-index: 10; }
.notifications-item:hover .speak-btn { display: flex; }
.speak-btn:hover { background: var(--pastel-blue); color: white; transform: scale(1.1); }
.speak-btn.speaking { animation: speaking-pulse 1.5s infinite; }

/* MEJORA 18: Estilos para Botón de "Marcar como Leído" */
.read-btn { position: absolute; top: 8px; left: 8px; background: rgba(255, 255, 255, 0.9); border: 1px solid var(--border-light); border-radius: 50%; width: 28px; height: 28px; display: none; align-items: center; justify-content: center; cursor: pointer; color: var(--text-secondary); font-size: 0.9rem; transition: var(--transition); z-index: 10; }
.notifications-item:hover .read-btn { display: flex; }
.read-btn:hover { background: var(--pastel-green); color: white; transform: scale(1.1); }

@keyframes slideIn { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }
@keyframes pulse { 0% { box-shadow: 0 0 0 0 rgba(255, 209, 102, 0.7); } 70% { box-shadow: 0 0 0 8px rgba(255, 209, 102, 0); } 100% { box-shadow: 0 0 0 0 rgba(255, 209, 102, 0); } }
@keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
@keyframes speaking-pulse { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.1); opacity: 0.8; } }

/* MEJORA 9: Micro-interacciones Suaves */
.notifications-item:hover { transform: translateY(-1px) scale(1.005); box-shadow: var(--shadow-md); }
@keyframes slideInRight { from { opacity: 0; transform: translateX(10px); } to { opacity: 1; transform: translateX(0); } }
.notifications-item.new { animation: slideInRight 0.25s ease-out; }

/* MEJORA 12: Estilos para Gestos Táctiles */
.notifications-item { position: relative; touch-action: pan-y; }
.notifications-item.swipe-out { opacity: 0.3; transform: translateX(-100%); transition: all 0.3s ease-out; }

@media (max-width: 576px) {
    .notification-panel { width: 320px; max-height: 450px; }
    .notification-tabs { gap: 4px; }
    .tab-item { min-width: 60px; padding: 6px 8px; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let previousCount = 0;
    let notificationsData = { sinAsignar: [], enProceso: [], revision: [], cerrados: [] };
    ...
    function setupEventListeners() {
        ...
        speechToggle.addEventListener('click', () => {
            ...
            speechIcon.style.color = speechEnabled ? 'var(--text-primary)' : 'var(--text-secondary)';
    ...
    function speak(text, buttonElement) {
        ...
        currentUtterance.onstart = () => { if (buttonElement) buttonElement.classList.add('speaking'); };
        currentUtterance.onend = () => { if (buttonElement) buttonElement.classList.remove('speaking'); };
    ...
    function archiveNotification(element) {
        ...
        fetch(`/soportes/notificaciones/${id}/archivar`, { method: 'POST' })
            .then(() => { 
    ...
    function mostrarIndicadorNuevasNotificaciones(data) {
        ...
        setTimeout(() => { notificationIndicator.classList.remove('show'); }, 5000);
    }
    
    function mostrarEstadoVacio(category) {
        const configuraciones = {
            sinAsignar: { icon: 'feather-user-x', message: 'No hay soportes sin asignar' },
            enProceso: { icon: 'feather-loader', message: 'No hay soportes en proceso' },
            revision: { icon: 'feather-eye', message: 'No hay soportes en revisión' },
            cerrados: { icon: 'feather-check-circle', message: 'No hay soportes cerrados' }
        };
        const config = configuraciones[category] || { icon: 'feather-info', message: 'No hay notificaciones' };
        listaNotificaciones.innerHTML = `
    ...
    function reproducirNotificacionSonora() {
        ...
        if (audioContext.state === 'suspended') {
            console.warn('AudioContext está suspendido. El sonido no se puede reproducir sin una interacción del usuario primero.');
            return;
        }
    ...
    function trackEvent(action, category = 'Notificaciones') {
        if (typeof gtag !== 'undefined') {
            gtag('event', action, { 'event_category': category, 'event_label': 'Centro de Soportes' });
        }
    ...
    function obtenerNotificacionesPorCategoria(category) {
        switch(category) {
            case 'sinAsignar': return notificationsData.sinAsignar;
            case 'enProceso': return notificationsData.enProceso;
            case 'revision': return notificationsData.revision;
            case 'cerrados': return notificationsData.cerrados;
            default: return [];
        }
    }
    ...
    function crearElementoNotificacion(item) {
        ...
        speakButton.addEventListener('click', (e) => { e.stopPropagation(); speak(textToSpeak, speakButton); });
    ...
    function markAsRead(id, element) {
        ...
        fetch(`/soportes/notificaciones/${id}/leer`, { method: 'POST' })
            .then(() => {
    ...
    function obtenerConfiguracionEstado(estadoId) {
        const configuraciones = {
            '1': { class: 'unassigned', icon: 'user-x' },
            '2': { class: 'inprogress', icon: 'loader' },
            '3': { class: 'review', icon: 'eye' },
            '4': { class: 'closed', icon: 'check-circle' }
        };
        return configuraciones[estadoId] || { class: 'default', icon: 'help-circle' };
    }
