...
class FlujoDeTrabajoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FlujoDeTrabajo::with(['usuario', 'area'])->withCount('correspondencias');
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($words) {
                foreach ($words as $word) {
                    $q->orWhere('nombre', 'LIKE', "%$word%")->orWhere('detalle', 'LIKE', "%$word%");
                }
            });
        }
        ...
        if (auth()->user()->hasPermission('correspondencia.flujotrabajo.todos')) {
            ...
            $conteoPorArea = FlujoDeTrabajo::selectRaw('id_area, COUNT(*) as total')->groupBy('id_area')->pluck('total', 'id_area');
            ...
            $areauserauth = auth()->user()->load('cargoRelation')->cargoRelation->GDO_area_id;
            $flujos = $query->where('id_area', $areauserauth)->paginate(15)->appends($request->all());
            $total_flujos = FlujoDeTrabajo::where('id_area', $areauserauth)->count();

            $areas_disponibles = GdoArea::where('id', $areauserauth)->get()->map(function ($area) use ($total_flujos) {
                    $area->flujos_count = $total_flujos;
            return $area;});
        }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('correspondencia.flujos.index')->with('success', 'Flujo de trabajo creado y asignado al área correctamente.');
    }
    ...
    public function show(FlujoDeTrabajo $flujo)
    {
        ...
        if ($total_finalizados > 0) {
            $tiempo_promedio_dias = $flujo->correspondencias()->where('finalizado', 1)->selectRaw('AVG(DATEDIFF(updated_at, created_at)) as promedio')->value('promedio');
        }
        ...
        return view('correspondencia.flujos.show', compact('flujo', 'total_activos', 'total_finalizados', 'tiempo_promedio_dias', 'labelsGrafica', 'datosGrafica'));
    }
    ...
    public function update(Request $request, FlujoDeTrabajo $flujo)
    {
        ...
        return redirect()->route('correspondencia.flujos.show', $flujo)->with('success', 'Flujo actualizado correctamente.');
    }
    ...
    public function destroy(FlujoDeTrabajo $flujo)
    {
        ...
        if ($flujo->correspondencias()->exists()) {
            return redirect()->back()->with('error', 'No se puede eliminar: Este flujo tiene registros de correspondencia vinculados.');
        }
